...
// Get a 64 bit signed integral value
LONGLONG CIni::GetInt64(LPCTSTR lpSection, LPCTSTR lpKey, LONGLONG nDefault, int nBase) const
{
	TCHAR sz[DEF_PROFILE_NUM_LEN + 1] = _T("");
	GetString(lpSection, lpKey, sz, DEF_PROFILE_NUM_LEN);
	return *sz == _T('\0') ? nDefault : LONGLONG(_tcstoi64(sz, NULL, __ValidateBase(nBase)));
}

// Get a 64 bit unsigned integral value
ULONGLONG CIni::GetUInt64(LPCTSTR lpSection, LPCTSTR lpKey, ULONGLONG nDefault, int nBase) const
{
	TCHAR sz[DEF_PROFILE_NUM_LEN + 1] = _T("");
	GetString(lpSection, lpKey, sz, DEF_PROFILE_NUM_LEN);
	return *sz == _T('\0') ? nDefault : ULONGLONG(_tcstoui64(sz, NULL, __ValidateBase(nBase)));
}
...
// Write a 64-bit signed integral value to the ini file
BOOL CIni::WriteInt64(LPCTSTR lpSection, LPCTSTR lpKey, LONGLONG nValue, int nBase) const
{
	TCHAR szValue[DEF_PROFILE_NUM_LEN + 1] = _T("");
...
// Write a 64-bit unsigned value to the ini file
BOOL CIni::WriteUInt64(LPCTSTR lpSection, LPCTSTR lpKey, ULONGLONG nValue, int nBase) const
{
	TCHAR szValue[DEF_PROFILE_NUM_LEN + 1] = _T("");
...
// Read a int64 from the ini file, increase it and then write it back to the ini file
BOOL CIni::IncreaseInt64(LPCTSTR lpSection, LPCTSTR lpKey, LONGLONG nIncrease, int nBase) const
{
	LONGLONG nVal = GetInt64(lpSection, lpKey, 0, nBase);
	nVal += nIncrease;
...
// Read a 64-bit UINT from the ini file, increase it and then write it back to the ini file
BOOL CIni::IncreaseUInt64(LPCTSTR lpSection, LPCTSTR lpKey, ULONGLONG nIncrease, int nBase) const
{
	ULONGLONG nVal = GetUInt64(lpSection, lpKey, 0, nBase);
	nVal += nIncrease;
...
// Convert a 64-bit integer into binary string format
void CIni::__UInt64ToBinaryString(ULONGLONG nNumber, LPTSTR lpBuffer, DWORD dwBufSize)
{
	if (dwBufSize == 0)
		return;
...
// Convert a 64-bit signed integer into string representation, based on its base
void CIni::__Int64ToString(LONGLONG nNumber, LPTSTR lpBuffer, int nBase)
{
	switch (nBase)
	{
	case BASE_BINARY:
	case BASE_OCTAL:
	case BASE_HEXADECIMAL:
		__UInt64ToString((ULONGLONG)nNumber, lpBuffer, nBase);
		break;
...
// Convert a 64-bit unsigned integer into string representation, based on its base
void CIni::__UInt64ToString(ULONGLONG nNumber, LPTSTR lpBuffer, int nBase)
{
	switch (nBase)
	{
	case BASE_BINARY:
		__UInt64ToBinaryString(nNumber, lpBuffer, DEF_PROFILE_NUM_LEN);
		break;
