...
int GetGUIDFormStrDevID(char *guid,WCHAR* pszDeviceId)
{
	int devIdLength=0;
	while(pszDeviceId[devIdLength]!=0)
		devIdLength++;
	int i,pos;
	for(pos=devIdLength-2,i=31;i>=0;i--,pos--)
	{
		if(pszDeviceId[pos]==L'-')
			pos--;
		if(i%2==1)
		{
			guid[i/2]=ConvertWCHAR2int(pszDeviceId[pos]);
		}
		else
		{
			guid[i/2]=(ConvertWCHAR2int(pszDeviceId[pos])<<4)|guid[i/2];
		}
	}
	swap(guid,guid+3);swap(guid+1,guid+2);
	swap(guid+4,guid+5);
	swap(guid+6,guid+7);
	return 1;
}
...
HRESULT EnumDevice(EDataFlow eDataFlow, UINT uNumElements, AUDIO_DEVICE_INFO *pDevicInfo)
{
    ...
    for (index = 0; index < dwCount; index++)
    {
		...
		GetGUIDFormStrDevID((char*)&pDevicInfo[index].DeviceGUID,pszDeviceId);
