				   ...
				   )  
{
    ...
    { 
        case DLL_PROCESS_ATTACH:
			...
			wVersionRequested = MAKEWORD( 1, 0 );		//使用winsock 1

			if (WSAStartup(wVersionRequested,&wsaData)!=0) 
			{
				OutputDebugString("failed to WSAStartup winsock 1 .\n");
				return FALSE;
			}
            ...
            if (hMapObject == NULL) 
			{
				OutputDebugString("failed to create share memory.\n");
				return FALSE; 
			}
			...
			{
				OutputDebugString("failed to  Get  pointer to the file-mapped shared memory.\n");
				return FALSE; 
			}
        ...
        case DLL_PROCESS_DETACH:
			...
			{			
				...
				{	
					...
					/**qhf: 由于函数ortp_exit会导致当前线程无限期阻塞，这里干脆注销掉，但存在内存泄漏危险*/
					ortp_exit();
					
					//UnregisterLog(&dwoRTPLogLevel, "LOG_ORTP");

					OutputDebugString("begin to Unmap  shared memory ...\n");

					// Unmap shared memory from the process's address space. 
					 ret=	UnmapViewOfFile(lpSharedData);
					if (ret==0)
					{
						OutputDebugString("failed to UnmapViewOfFile\n");
					}
