...
////////////////////////////////////////////////////////////////////////////////////////////////////
/// @fn static ortp_socket_t create_and_bind(const char *addr, int port, int *sock_family,
///     bool_t reuse_addr)
///
/// @brief   修改原来的代码以支持组播，addr地址可以设为组播地址
///
/// @author modify by   qhf
/// @date   2010/5/28
///
/// @param  addr                The address.
/// @param  port                The port.
/// @param [in,out] sock_family If non-null, the sock family.
/// @param  reuse_addr          The reuse address.
///
/// @return .
////////////////////////////////////////////////////////////////////////////////////////////////////
static ortp_socket_t create_and_bind(const char* addr, int port, int* sock_family, bool_t reuse_addr)
{
    ...
    bool_t isMulticastAddr = FALSE;  //表示addr是否组播地址
    int port_bynetworkorder = htons(port);
    ...
    for(res = res0; res; res = res->ai_next)
    {
        ...
        *sock_family = res->ai_family;
        // 判断是否组播地址，如果是组播地址，则绑定本地地址
        isMulticastAddr = FALSE;
        //#ifndef __hpux  //组播
        //#endif /*hpux*/
        switch(res->ai_family)
        {
          case AF_INET:
            //ipv4
            {
                struct sockaddr_in localsaddr;
                localsaddr.sin_family = AF_INET;
                localsaddr.sin_port = port_bynetworkorder;
                localsaddr.sin_addr.s_addr = INADDR_ANY;  //直接设为本机地址
                //绑定socket到本地端口
                err = bind(sock, (struct sockaddr *)&localsaddr, sizeof(localsaddr));
                if(err != 0)
                {
                    ortp_warning("Fail to bind rtp socket to ipv4 localaddress port=%i : %s.", port, getSocketError());
                    close_socket(sock);
                    sock = -1;
                    continue;
                }
                if(IN_MULTICAST(ntohl(((struct sockaddr_in *)res->ai_addr)->sin_addr.s_addr)))
                {
                    ...
                    if(err < 0)
                    {
                        ortp_warning("Fail to join multicast group: %s.", getSocketError());
                        close_socket(sock);
                        sock = -1;
                        continue;
                    }
          ...
          case AF_INET6:
            //ipv6
            {
                struct   sockaddr_in6 localsaddr;
                memset((char*)(&localsaddr), 0, sizeof(localsaddr));
                localsaddr.sin6_family = AF_INET6;
                localsaddr.sin6_port = port_bynetworkorder;
                //没有必要设置sin.sin6_addr为 0了 因为memset(sin)的时候会顺便设定的
                // localsaddr.sin6_addr.s6_addr = INADDR6_ANY;
                //绑定socket到本地端口
                err = bind(sock, (struct sockaddr *)&localsaddr, sizeof(localsaddr));
                //err = bind (sock,  res->ai_addr, res->ai_addrlen);
                ortp_error("==>binding rtp socket to ipv6 localaddress:port = %s : %d.", addr, port);
                if(err != 0)
                {
                    ortp_error("Fail to bind rtp socket to ipv6 localaddress port=%i : %s.", port, getSocketError());
                    close_socket(sock);
                    sock = -1;
                    continue;
                }
                if(IN6_IS_ADDR_MULTICAST(&(((struct sockaddr_in6 *)res->ai_addr)->sin6_addr)))
                {
                    ...
                    if(err < 0)
                    {
                        ...
                        ortp_message("success to join address group:%s");
                }
            }
            break;
          default:
...
#else
    {
        ...
        *sock_family = AF_INET;
        /* err = inet_aton(addr, &saddr.sin_addr);  //赋值
         if (err < 0)
         {
        ...
        saddr.sin_port = port_bynetworkorder;
        ...
        if(IN_MULTICAST(ntohl(saddr.sin_addr.s_addr)))
        {
            ...
            //加入组播组
            mreq.imr_multiaddr.s_addr = saddr.sin_addr.s_addr;
            mreq.imr_interface.s_addr = INADDR_ANY;
            err = setsockopt(sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (SOCKET_OPTION_VALUE)&mreq, sizeof(mreq));
            if(err < 0)
            {
...
int rtp_session_set_multicast_ttl(RtpSession* session, int ttl)
{
    ...
    switch(session->rtp.sockfamily)
    {
      case AF_INET:
        {
            retval = setsockopt(session->rtp.socket, IPPROTO_IP, IP_MULTICAST_TTL, (SOCKET_OPTION_VALUE)&session->multicast_ttl, sizeof(session->multicast_ttl));

            if(retval < 0)
                break;

            retval = setsockopt(session->rtcp.socket, IPPROTO_IP, IP_MULTICAST_TTL, (SOCKET_OPTION_VALUE)&session->multicast_ttl, sizeof(session->multicast_ttl));
        }
        break;
#ifdef ORTP_INET6
      case AF_INET6:
        {
            retval = setsockopt(session->rtp.socket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (SOCKET_OPTION_VALUE)&session->multicast_ttl, sizeof(session->multicast_ttl));

            if(retval < 0)
                break;

            retval = setsockopt(session->rtcp.socket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (SOCKET_OPTION_VALUE)&session->multicast_ttl, sizeof(session->multicast_ttl));
        }
        break;
#endif
      default:
...
int rtp_session_set_multicast_loopback(RtpSession* session, int yesno)
{
    ...
    switch(session->rtp.sockfamily)
    {
      case AF_INET:
        {
            retval = setsockopt(session->rtp.socket, IPPROTO_IP, IP_MULTICAST_LOOP, (SOCKET_OPTION_VALUE)&session->multicast_loopback, sizeof(session->multicast_loopback));

            if(retval < 0)
                break;

            retval = setsockopt(session->rtcp.socket, IPPROTO_IP, IP_MULTICAST_LOOP, (SOCKET_OPTION_VALUE)&session->multicast_loopback, sizeof(session->multicast_loopback));
        }
        break;
#ifdef ORTP_INET6
      case AF_INET6:
        {
            retval = setsockopt(session->rtp.socket, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, (SOCKET_OPTION_VALUE)&session->multicast_loopback, sizeof(session->multicast_loopback));

            if(retval < 0)
                break;

            retval = setsockopt(session->rtcp.socket, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, (SOCKET_OPTION_VALUE)&session->multicast_loopback, sizeof(session->multicast_loopback));
        }
        break;
#endif
      default:
...
int rtp_session_set_dscp(RtpSession* session, int dscp)
{
    ...
    if(ovi.dwMajorVersion > 5)
    {
        ...
        else
        {
            ...
            {
                BOOL QoSResult;
                QoSResult = QOSAddSocketToFlow(session->rtp.QoSHandle,
                                               session->rtp.socket,
...
#   ifdef IPV6_TCLASS /*seems not defined by my libc*/
            retval = setsockopt(session->rtp.socket, IPPROTO_IPV6, IPV6_TCLASS, (SOCKET_OPTION_VALUE)&tos, sizeof(tos));
#   else
            /*in case that works:*/
            retval = setsockopt(session->rtp.socket, IPPROTO_IPV6, IP_TOS, (SOCKET_OPTION_VALUE)&tos, sizeof(tos));
#   endif
            break;
#endif
          default:
...
int rtp_session_set_remote_addr_full(RtpSession* session, const char* addr, int rtp_port, int rtcp_port)
{
    ...
    memcpy(&session->rtcp.rem_addr, &session->rtp.rem_addr, sizeof(struct sockaddr_in));
...
{
    if(rtpfd >= 0)
        set_non_blocking_socket(rtpfd);
    if(rtcpfd >= 0)
        set_non_blocking_socket(rtcpfd);
...
int rtp_session_rtp_send(RtpSession* session, mblk_t* m)
{
    ...
    else
    {
		...
		/* 主动丢包*/
		{
			static int count=0;
			count ++;
			if (count % ((int)(1/0.03))==0)
				error =  (int)(m->b_wptr - m->b_rptr);
			else
    ...
    if(error < 0)
    {
        ...
        if(session->on_network_error.count > 0)
        {
            ...
            ortp_warning("Error sending rtp packet: %s ; socket=%i", getWinSocketError(session->rtp.send_errno), sockfd);
    }
    else
    {
...
int rtp_session_rtcp_send(RtpSession* session, mblk_t* m)
{
    ...
    if(session->rtcp.enabled && ((sockfd >= 0 && (session->rtcp.rem_addrlen > 0 || using_connected_socket)) || rtp_session_using_transport(session, rtcp)))
    {
        if(rtp_session_using_transport(session, rtcp))
        {
            error = (session->rtcp.tr->t_sendto) (session->rtcp.tr, m, 0, destaddr, destlen);
        }
        else
        {
#ifdef USE_SENDMSG
        ...
        if(error < 0)
        {
            ...
            if(session->on_network_error.count > 0)
            {
                ...
                ortp_warning("Error sending rtcp packet: %s ; socket=%i; addr=%s",
                             getSocketError(),
                             session->rtcp.socket,
        ...
        ortp_debug("Not sending rtcp report: sockfd=%i, rem_addrlen=%i, connected=%i", sockfd, session->rtcp.rem_addrlen, using_connected_socket);
    freemsg(m);
    return error;
}
...
int rtp_session_rtp_recv(RtpSession* session, uint32_t user_ts)
{
    ...
    //qhf: 若使用while，当socket处于阻塞模式时，则在此次会产生无限循环
    //while (1)      //注释掉 -- modified by qhf
    {
        int bufsz;
        ...
        if(sock_connected)
        {
            ...
            error = (session->rtp.tr->t_recvfrom)(session->rtp.tr, mp, 0, (struct sockaddr *)&remaddr, & addrlen);
        else
            error = recvfrom(sockfd, (char*)mp->b_wptr, bufsz, 0, (struct sockaddr *)&remaddr, &addrlen);
        if(error > 0)
        {
        ...
        else
        {
			...
			else if (is_would_timeout_error(errnum))
			{
				//timeout
				ortp_debug("Error receiving RTP packet: timeout error, error code: %s.", getSocketError());
			}
            else if(!is_would_block_error(errnum))
            {
                if(session->on_network_error.count > 0)
                {
                    ...
                    ortp_warning("Error receiving RTP packet: %s.", getSocketError());
            }
...
int rtp_session_rtcp_recv(RtpSession* session)
{
    ...
    if(session->rtcp.socket < 0 && !rtp_session_using_transport(session, rtcp))
        return -1; /*session has no rtcp sockets for the moment*/

     //RTCP使用非阻塞接收，不能使用阻塞，否则发送端发不了数据？ -- modified by qhf
	//while (1)     
    {
        ...
        else
        {
            ...
            if(rtp_session_using_transport(session, rtcp))
                error = (session->rtcp.tr->t_recvfrom)(session->rtcp.tr, mp, 0, (struct sockaddr *)&remaddr, & addrlen);
            else
                error = recvfrom(session->rtcp.socket, (char*)mp->b_wptr, RTCP_MAX_RECV_BUFSIZE, 0, (struct sockaddr *)&remaddr, &addrlen);
        }
        if(error > 0)
        {
            mp->b_wptr += error;
            /* post an event to notify the application*/
            {
                ...
                //  -   收到报告包后记录接收信息
                rtp_session_rtcp_parse(session, mp);
                //--------------END---------------------
				//新增注释：可以在rtp_session_rtcp_record函数中计算RTCP包的丢包、延时、抖动等参数，并保存到RTP Session中，外部可以读取其值，这样便不需要
				// 调用rtp_session_notify_inc_rtcp函数拷贝分发RTCP接收事件，也有利于提供性能
        ...
        else
        {
            ...
            if(error == 0)
            {
                ortp_warning("rtcp_recv: strange... recv() returned zero.");
            }
			else if (is_would_timeout_error(errnum))
			{
				//timeout
				ortp_debug("Error receiving RTCP packet: timeout error, error code: %s.", getSocketError());
			}
            else if(!is_would_block_error(errnum))
            {
                if(session->on_network_error.count > 0)
                {
                    rtp_signal_table_emit3(&session->on_network_error, (long)"signal: Error receiving RTCP packet", INT_TO_POINTER(errnum));
                }
