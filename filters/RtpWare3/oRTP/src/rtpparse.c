...
//#endif
    ...
    /* update some statistics */
    {
        ...
        else if (rtp->seq_number < 200 && extseq->split.lo > (65536 - 200))
        {
	...
	//qhf: 使用绝对时间戳对Directshow时间戳的抖动计算可能会精确些
	 //jitter_control_new_packet(&session->rtp.jittctl, rtp->timestamp, local_str_ts);
   jitter_control_new_packet(&session->rtp.jittctl, rtp->timestamp,  convertTimevalToMS(&session->last_recv_time));

	//qhf:	这里有时间戳比较，可能会产生一些问题，所以先注释掉
