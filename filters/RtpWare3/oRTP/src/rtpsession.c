...
void wait_point_wakeup_at(WaitPoint* wp, uint32_t t, bool_t dosleep)
{
    ...
    wp->wakeup = TRUE;
    if(dosleep)
        ortp_cond_wait(&wp->cond, &wp->lock);
}
...
void rtp_session_init(RtpSession* session, int mode)
{
    ...
    if((mode == RTP_SESSION_SENDONLY) || (mode == RTP_SESSION_SENDRECV))
    {
        ...
        rtp_session_set_source_description(session, "unknown@unknown", NULL, NULL, NULL, NULL, "oRTP-" ORTP_VERSION, "This is free sofware (LGPL) !");
    }
	...
	/************************************************************************/
	//设置最大接收包数 add by qhf
	session->rtp.max_rq_size = RTP_SESSION_DEFAULT_MAX_RTP_QUEUE_SIZE;
	session->rtcp.max_rq_size = RTP_SESSION_DEFAULT_MAX_RTCP_QUEUE_SIZE;
	/************************************************************************/
    ...
    jbp.max_size = -1;
    jbp.max_packets = 1000; /* maximum number of packet allowed to be queued */
    jbp.adaptive = TRUE;
...
void rtp_session_set_scheduling_mode(RtpSession* session, int yesno)
{
    if(yesno)
    {
        ...
        if(sched != NULL)
        {
            ...
            ortp_warning("rtp_session_set_scheduling_mode: Cannot use scheduled mode because the " "scheduler is not started. Use ortp_scheduler_init() before.");
    }
    else
...
int __rtp_session_sendm_with_ts(RtpSession* session, mblk_t* mp, uint32_t packet_ts, uint32_t send_ts)
{
    ...
    if(mp == NULL)
    {
        /*for people who just want to be blocked but
...
mblk_t* rtp_session_recvm_with_ts(RtpSession* session, uint32_t user_ts)
{
    ...
    if(session->flags & RTP_SESSION_SCHEDULED)
    {
        ...
        packet_time = rtp_session_ts_to_time(session, user_ts - session->rtp.rcv_query_ts_offset) + session->rtp.rcv_time_offset;
...
void rtp_session_dispatch_event(RtpSession* session, OrtpEvent* ev)
{
    ...
    {
		OrtpEvQueue * eq=(OrtpEvQueue *)it->data;
        ortp_ev_queue_put(eq, ortp_event_dup(ev));
		/************************************************************************/
		/* 判断是否包数较多，多则删除最旧的 --add by qhf            */
		if (eq->q.q_mcount>session->rtcp.max_rq_size)
		{
			OrtpEvent *	evptr = ortp_ev_queue_get(eq);
			if (evptr)
			{
			ortp_debug("OrtpEvent too much, discard one oldest.");
			ortp_event_destroy(evptr);
			}
		}	
		/************************************************************************/
    }
...
void rtp_session_uninit(RtpSession* session)
{
    ...
    if(session->rtp.cached_mp != NULL)
        freemsg(session->rtp.cached_mp);
    if(session->rtcp.cached_mp != NULL)
        freemsg(session->rtcp.cached_mp);
    if(session->sd != NULL)
        freemsg(session->sd);
...
static float compute_bw(struct timeval* orig, unsigned int bytes)
{
    ...
    time = (float)(current.tv_sec - orig->tv_sec) + ((float)(current.tv_usec - orig->tv_usec) * 1e-6);
