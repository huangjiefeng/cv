...
//将timeval结构时间转换为毫秒
inline uint32_t  convertTimevalToMS(struct timeval *tv)
{
    return  tv->tv_sec*1000+ tv->tv_usec /1000;
}

//将timeval结构时间转换为NTP时间的中间32位
inline  uint32_t convetTimevalToNTP32(struct timeval *tv)
{
    return (((uint32_t)tv->tv_sec + 0x83AA7E80) << 16) |             /* 0x83AA7E80 is the number of seconds from 1900 to 1970 */
        (((uint32_t)((double)tv->tv_usec * (double)(1LL << 32) * 1.0e-6) ) >> 16);      
}

//由NTP32转换单位为ms。
inline  double convetNTP32ToMS(uint32_t ntp32)
{
     //return (ntp32 >> 16) * 1000+ ((ntp32 & 0xffff) * 1000 ) /65536.0;
        return   ((double)ntp32 /65536.0 ) * 1000; 
}

//由NTP32转换单位为ms。
inline  uint32_t convetMSToNTP32(double sec)
{
    return (uint32_t)(sec * 65536.0);
}
