...
#define  RTP_SESSION_DEFAULT_MAX_RTP_QUEUE_SIZE	100   //rtp session最多保存接收到的RTP包数目		--add by qhf
#define  RTP_SESSION_DEFAULT_MAX_RTCP_QUEUE_SIZE	100   //rtp session最多保存接收到的RTCP包数目	--add by qhf
...
typedef struct _RtpStream
{
	...
	struct timeval last_rcv_RR_time;	/* time at which last RR was received. */
	uint32_t last_rtt;			/*last calculated round trip time.*/
	//-------------------END------------------------------
  //---------------Add by Liaokz. 2015-01-08---------------
  uint64_t last_rcv_SR_ntp_ts; /* NTP timestamp (64 bits) of last received SR */
  uint32_t last_rcv_SR_rtp_ts; /* stream timestamp corresponds to the NTP timestamp above */
  //-------------------END------------------------------
	...
	/* 补充绝对时间  -- by qhf                                                                     */
	/************************************************************************/
	uint32_t last_rtcp_report_snt_sr_t;	/* the time of the last rtcp sr sent, in absolute time, ms unit */
	uint32_t last_rtcp_report_snt_rr_t;	/* the time of the last rtcp rr sent, in absolute time, ms unit */
...
typedef struct _RtcpStream
{
	...
	int max_rq_size;   /* maximum number of packet allowed to be queued */
...
//inital a report block from rtpsession
