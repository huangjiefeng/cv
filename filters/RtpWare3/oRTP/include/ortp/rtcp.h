...
//    -     定义了扩展块，以及从SR报告包中获取报告块的方法
typedef struct report_extension
{
	uint32_t delay;
}report_extension_t;
...
typedef struct rtcp_sr{
	...
	//    -     根据RFC3550，报告包可以在尾部有一个扩展部分。在此假定如果SR报告包没有report block，那么它可能含有一个扩展部分，用于保存网络延时
	union{
		report_block_t rb[1];
		report_extension_t ext;
	};
