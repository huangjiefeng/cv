...
const AMOVIESETUP_FILTER sudInfTee =
{
    ...
    L"Multi Smart Tee",    // Filter's name
...
CBasePin *CTee::GetPin(int n)
{
	...
	CBaseOutputPin * pResult;
    pResult=GetPinNFromList(n - 1);
    return  pResult;
}
...
STDMETHODIMP CTee::NonDelegatingQueryInterface(REFIID riid, void ** ppv)
{
	/* Do we have this interface */

	if (riid == IID_IMULTISMARTTEE) {
		return GetInterface((IMULTISMARTTEE *) this, ppv);
...
HRESULT CTee::GetFreePin(IPin ** ppIPin)
{
	int n = 0;
	POSITION pos = m_OutputPinsList.GetHeadPosition();
	...
	while(pos) 
	{
		...
		if (pOutputPin && pOutputPin->m_Connected == NULL)
		{
			pOutputPin->NonDelegatingQueryInterface(IID_IPin,(void **)&pIPin);
			* ppIPin=pIPin;
			//pIPin->Release();
			return S_OK;
		}
	}
//	output<<"return "<<pOutputPin;
	return E_FAIL;
}
HRESULT CTee::IsConnect(int * result)
{
	...
	{
		pOutputPin = m_OutputPinsList.GetNext(pos);
		if (pOutputPin && pOutputPin->m_Connected != NULL)  //спа╛╫с
		{
			* result= 1;
			return S_OK;
		}
...
// GetPinNFromList
//
CBaseOutputPin *  CTee::GetPinNFromList(int n)
{
...
HRESULT CTee::ChangeOutputPinState(IBaseFilter * pDownstreamFilter,bool bState)
{
    ...
    while(n)
    {
        ...
        if(pOutputPin != NULL)
        {
			...
			if(pOutputPin->pDownstreamFilter == pDownstreamFilter)
			{		
				pOutputPin->m_bStart=bState;	
				return S_OK;
			}
...
HRESULT CTee::StartTransfer(IBaseFilter * pDownstreamFilter)
{
	return ChangeOutputPinState(pDownstreamFilter,true);
}

HRESULT CTee::StopTransfer(IBaseFilter * pDownstreamFilter)
{
	return ChangeOutputPinState(pDownstreamFilter,false);
}

HRESULT CTee::SetOverallTransfer( bool transfer )
{
    CAutoLock lock_it(m_pLock);
    m_bTransfer = transfer;
    return S_OK;
}
...
HRESULT CTeeOutputPin::CompleteConnect(IPin *pReceivePin)
{
	...
	pDownstreamFilter=PinInfo.pFilter;
	...
	if(m_Connected != NULL && m_pOutputQueue == NULL)
    {
