...
H264ByteStreamHandler::WriteDescriptor(Atom* patm, int id, int dataref, long scale)
{
    ...
    // Store the actual size -- By Liaokz
    if (seq.EncodedWidth() > 0)
    {
        m_cx = seq.EncodedWidth();
    }
    if (seq.EncodedHeight() > 0)
    {
        m_cy = seq.EncodedHeight();
        if (m_cy == 1088) m_cy = 1080; // 修正，参见H.264规范
    }
    // ---End by Liaokz
