...
MovieWriter::Close(REFERENCE_TIME* pDuration)
{
	...
	unsigned int nEmptyTrack = 0; // empty track count - Add by Liaokz
    for (it = m_Tracks.begin(); it != m_Tracks.end(); it++)
    {
        TrackWriter* pTrack = *it;
		// DO NOT write empty track - Add by Liaokz
		if (pTrack->Length() == 0)
		{
			++nEmptyTrack;
			continue;
	...
	if (nEmptyTrack == m_Tracks.size()) // Add by Liaokz
	{
		return S_FALSE;
	}
    ...
    *pDuration = tDur;
    ...
    b[cHdr + 33] = 0x01;
    b[cHdr + 48] = 0x40;
    WriteLong((long)m_Tracks.size() - nEmptyTrack + 1, b + cHdr + 76); // one-based next-track-id - Modified by Liaokz
...
MovieWriter::CheckQueues()
{
  ...
  //          break;
  //      }
		// 实时音视频流，无法保证音视频按时到达，且有时发送端不发送音频
		// 因此不在此处做interleaving，我们将在后期调用专门的工具
...
// Note: if m_pMovie == NULL, this method will not be called
//       see Mpeg4Mux::OnReceiveSample for details
HRESULT 
...
TrackWriter::IndexSample(bool bSync, REFERENCE_TIME tStart, REFERENCE_TIME tStop, long cBytes)
{
	...
	// Add By Liaokz
	if (m_rtStart == 0) { m_rtStart = tStart; }
	if (m_rtEnd < tStop) { m_rtEnd = tStop; }
	m_rtTime += (tStop - tStart);
}
...
TrackWriter::Close(Atom* patm)
{
    ...
    // For debug -- Add by Liaokz
    REFERENCE_TIME tDur = (m_rtEnd-m_rtStart)/10000;
	if (m_pType->IsAudio()) {
		LOG((TEXT("Track[%d] Audio track duration=%I64dms"),
			m_index, tDur));
	} else {
		LOG((TEXT("Track[%d] Video track duration=%I64dms"),
			m_index, tDur));
	}
...
MediaChunk::Write(Atom* patm)
{
	...
	DbgLog((LOG_TRACE, 0, TEXT("%d: Pos %d KB, at %d dur %d ms, %d KB"), 
		m_pTrack->ID(), long(posChunk/1024), long(m_tStart/10000), long(GetDuration() / 10000), long(Length() / 1024)));

	if (m_bOldIndexFormat)
	{
		long cBytes = 0;
		const int max_bytes = 22050;
...
DurationIndex::Add(REFERENCE_TIME tStart, REFERENCE_TIME tEnd)
{
	...
	if (m_nSamples < mode_decide_count)
	{
		...
		m_nSamples++;
		m_tStartLast = tStart; // Add By Liaokz
		m_tStopLast = tEnd;
		return;
	}
	else if (m_nSamples == mode_decide_count)
	{
		// For adding elst entries. --By Liaokz, 2013-5
		m_tElstFirst = m_SampleStarts[0];
		m_tElstStartDur = 0;
		m_tElstRefDuration = m_SumDurations;
		m_tElstAveFrame = m_SumDurations / m_nSamples;
	...
	{
        //AddDuration(long(ToScale(tStart) - m_TotalDuration)); // Orig code
		// For adding elst entries. --By Liaokz, 2013-5
		if (m_bUseElst && tStart - m_tStopLast >= m_tElstAveFrame * 10)
		{
			// Empty edit list found
			AddDuration(long(ToScale(m_tStopLast - m_tStartLast)));
			m_TotalDuration += long(ToScale(tStart) - m_TotalDuration - ToScale(m_tStopLast - m_tStartLast));

			m_tElstRefDuration += m_tStopLast - m_tStartLast;
			//REFERENCE_TIME tDurMovie = tStart - m_tElstFirst; // For implicit empty edit
			REFERENCE_TIME tDurMovie = m_tStopLast - m_tElstFirst; // For explicit empty edit
			m_ELST.push_back(tDurMovie);
			m_ELST.push_back(m_tElstStartDur);
			// Add an explicit empty edit
			REFERENCE_TIME tDurEmpty = tStart - m_tStopLast;
			m_ELST.push_back(tDurEmpty);
			m_ELST.push_back((REFERENCE_TIME)-1);
			// Update time
			m_tElstFirst = tStart;
			m_tElstStartDur = m_tElstRefDuration;
		}
		else
		{
			AddDuration(long(ToScale(tStart) - m_TotalDuration));
			m_tElstRefDuration += tStart - m_tStartLast;
		}
		// End for elst
    }
...
{
    if (m_tStartFirst > 0 || m_ELST.size() > 0) // For elst entries. By Liaokz
    {
		...
		// Last edit list. By Liaokz
		REFERENCE_TIME tDurMovie = m_tStopLast - m_tElstFirst;
		m_ELST.push_back(tDurMovie);
		m_ELST.push_back(m_tElstStartDur);

		const int cSzMax = 28 + 20 * m_ELST.size();
		BYTE * b = new BYTE[cSzMax];
		ZeroMemory(b, cSzMax);
		// End by Liaokz
       ...
       /* BYTE b[48];
        ...
        if ((offset > 0x7fffffff) || (dur > 0x7fffffff))
        {
            ...
            b[45] = 1;
            cSz = 48;*/
			for (unsigned int i = 0; i < m_ELST.size(); i+=2)
			{
				WriteI64(m_ELST[i] * scale / UNITS, b+cSz);
				if ((REFERENCE_TIME)-1 == m_ELST[i+1])
				{
					WriteI64((LONGLONG)-1, b+cSz+8);
				} 
				else
				{
					WriteI64(ToScale(m_ELST[i+1]), b+cSz+8);
				}
				b[cSz+17] = 1;
				cSz += 20;
			}
        ...
        else
        {
			...
			{
				//LOG_PRINT("%I64d %I64d\n", m_ELST[i], m_ELST[i+1]);
				WriteLong(long(m_ELST[i] * scale / UNITS), b+cSz);
				if ((REFERENCE_TIME)-1 == m_ELST[i+1])
				{
					WriteLong(-1, b+cSz+4);
				}
				...
				{
					WriteLong(long(ToScale(m_ELST[i+1])), b+cSz+4);
				}
				b[cSz+9] = 1;
				cSz += 12;
			}
...
SyncIndex::Write(Atom* patm)
{
	...
	// Modify: 没有关键帧列表就不创建stss，因为音频默认无关键帧，空stss无法被正确定位。
	// 当all syncs时，也不会记录关键帧，故可视为同情况。 --By Liaokz, 2013-5-4
    if (/*!m_bAllSync*/m_Syncs.Entries() > 0)
    {
