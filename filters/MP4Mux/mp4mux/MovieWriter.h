...
public:
    ...
    {
		// if clause add by Liaokz.
		if (!m_bUseElst)
		{
			tAdjust = -m_tStartFirst; // Track will start at 0
		} 

		m_tStartFirst += tAdjust; // Orig start
		m_tStartLast += tAdjust;
		m_tStopLast += tAdjust;
		m_TotalDuration += ToScale(tAdjust);
		m_refDuration += tAdjust;// orig end
		m_tElstFirst += tAdjust; // Add By Liaokz
    }
...
private:
	...
	// ELST entries. --By Liaokz, 2013-5
	std::vector<REFERENCE_TIME> m_ELST;
	REFERENCE_TIME m_tElstFirst;
	REFERENCE_TIME m_tElstStartDur;
	REFERENCE_TIME m_tElstRefDuration;
	REFERENCE_TIME m_tElstAveFrame;
	bool m_bUseElst; // Decide whether use multiple entries
};
...
public:
	...
	bool HasOldIndex()
	{
		if (m_pType)
		{
			return m_pType->IsOldIndexFormat();
		}
		return false;
	}
	// Add by Liaokz
...
private:
	...
	// For logger - Add by Liaokz
	REFERENCE_TIME m_rtStart;
	REFERENCE_TIME m_rtEnd;
	REFERENCE_TIME m_rtTime; // the actual duration, calculated by sum the (tStop-tStart) of each samples

	// For empty track detect - Add by Liaokz
	long m_lLength;
};
