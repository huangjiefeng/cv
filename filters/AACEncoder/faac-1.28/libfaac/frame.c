                          ...
                          )
{
    ...
    for (channel = 0; channel < numChannels; channel++) 
	{
        ...
        //以下三行代码用于减少编码延迟, Liaokz, 2014-4
        if (aacObjectType == LTP) {
            //LPT模式下必须缓存一帧才能编码
            tmp = hEncoder->nextSampleBuff[channel];
            hEncoder->nextSampleBuff[channel] = hEncoder->next3SampleBuff[channel];
            hEncoder->next3SampleBuff[channel] = tmp;
        } else {
            tmp = hEncoder->sampleBuff[channel];
    ...
    //以下条件判断原值为3，更改以减少延迟，LPT模式至少缓存一帧, Liaokz, 2014-4
    if (hEncoder->frameNum <= (aacObjectType == LTP ? 1 : 0)) /* Still filling up the buffers */
        return 0;
