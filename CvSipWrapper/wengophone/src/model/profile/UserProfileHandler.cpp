...
void UserProfileHandler::setCurrentUserProfile(const std::string & name, const SipAccount & sipAccount,bool autoConnect) {
...
void UserProfileHandler::setLastUsedUserProfile(const UserProfile & userProfile) {
	...
	if(config.getPasswordRemember())
		...
		config.set(Config::PROFILE_LAST_USED_NAME_KEY, name);
}
...
void UserProfileHandler::init() {
	...
	bool autoConnect = config.getAutoConnect();

	setCurrentUserProfile(profileName, SipAccount::empty,autoConnect);
}

void UserProfileHandler::saveUserProfile(UserProfile & userProfile) {
	if (_autoSave && &userProfile) {
		UserProfileFileStorage userProfileStorage(userProfile);
