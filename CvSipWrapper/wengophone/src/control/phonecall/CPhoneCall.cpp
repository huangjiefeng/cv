...
void CPhoneCall::videoFrameReceivedEventHandler(PhoneCall & sender, void * video_hwnd, piximage * localVideoFrame) {
	typedef ThreadEvent2<void (void *, piximage *), void *, piximage *> MyThreadEvent;
	MyThreadEvent * event = new MyThreadEvent(boost::bind(&CPhoneCall::videoFrameReceivedEventHandlerThreadSafe, this, _1, _2), video_hwnd, localVideoFrame);
	PFactory::postEvent(event);
}
...
void CPhoneCall::videoFrameReceivedEventHandlerThreadSafe(void * video_hwnd, piximage * localVideoFrame) {
	if (_pPhoneCall) {
		_pPhoneCall->videoFrameReceivedEvent(video_hwnd, localVideoFrame);
	}
}
