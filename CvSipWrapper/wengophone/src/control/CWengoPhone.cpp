...
CWengoPhone::CWengoPhone(WengoPhone & wengoPhone, bool runInBackground)
	...
	_qtWengoPhone = (QtWengoPhone *)PFactory::getFactory().createPresentationWengoPhone(*this, runInBackground);//Modify By LZY 2010-09-27
...
void CWengoPhone::showAddContactEventHandlerThreadSafe(ContactInfo contactInfo) {
	if (_qtWengoPhone) {
		_qtWengoPhone->showAddContact(contactInfo);
	}
}
...
void CWengoPhone::bringMainWindowToFrontEventHandlerThreadSafe() {
	if (_qtWengoPhone) {
		_qtWengoPhone->bringMainWindowToFront();
	}
}
