...
QtWengoPhone * QtFactory::createPresentationWengoPhone(CWengoPhone & cWengoPhone, bool runInBackground) {
...
PContactList * QtFactory::createPresentationContactList(CContactList & cContactList) {
	if (!_qtContactList) {
		if (!_cWengoPhone) {
			LOG_FATAL("QtWengoPhone must be created before PContactList");
		}
