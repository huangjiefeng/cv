...
QtConferenceCallWidget::QtConferenceCallWidget(QWidget * parent, CWengoPhone & cWengoPhone, IPhoneLine * phoneLine , bool startConference)
	...
	_ui->label_2->hide();
	_ui->phoneNumber2LineEdit->hide();
	_ui->peer2ToolButton->hide();

	//对Sip帐号的输入内容进行过滤
	QRegExp exgExp(tr("^([0-9a-zA-Z]+([\\._-][0-9a-zA-Z]+)*@[0-9a-zA-Z]+(\\.[0-9a-zA- Z]+)*)+([/0-9a-zA-Z]+([\\._-][0-9a-zA-Z]+)*@[0-9a-zA-Z]+(\\.[0-9a-zA-Z]+)*)*$"));
	_ui->phoneNumber1LineEdit->setValidator( new QRegExpValidator( exgExp , this));
}
...
void QtConferenceCallWidget::setFirstPeer(std::string peer) {
...
void QtConferenceCallWidget::startClicked() {
	...
	if ((!_ui->phoneNumber1LineEdit->text().isEmpty()) /*&& (!_ui->phoneNumber2LineEdit->text().isEmpty())*/) 
	{
		...
		_cWengoPhone.getCUserProfileHandler().getCUserProfile()->makeCall(_ui->phoneNumber1LineEdit->text().toStdString());
		accept();
...
QString QtConferenceCallWidget::getSecondPeer()
{
