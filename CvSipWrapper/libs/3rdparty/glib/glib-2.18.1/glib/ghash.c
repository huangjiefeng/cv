                          ...
                          guint         *hash_return)
{
  ...
  if ( 0 == hash_table->size) 
  {
	  return NULL;///如果size为0,就直接返回NULL.
  }
  node_ptr = &hash_table->nodes[hash_value % hash_table->size];
...
{
  //GHashNode *node;///s-h
  GHashNode **node_ptr, *node;///s-h,增加一个node_ptr的声明；

  g_return_val_if_fail (hash_table != NULL, NULL);

  //node = *g_hash_table_lookup_node (hash_table, key, NULL);///此处如果是空指针，则导致异常
	node_ptr= g_hash_table_lookup_node (hash_table, key, NULL);///先查找结点
	if (!node_ptr){
		return NULL; ///如果结点指针是空，则直接返回NULL
	}
	node = *node_ptr;///如果结点非空，可以提取结点内容
                              ...
                              gpointer      *value)
{
  ...
  //node = *g_hash_table_lookup_node (hash_table, lookup_key, NULL);///s-h,此处如果是空指针，则导致异常.
  node_ptr= g_hash_table_lookup_node (hash_table, lookup_key, NULL);///先查找结点.
  if (!node_ptr){
  	return FALSE; ///如果结点指针是空，则直接返回FALSE.
  }
  node = *node_ptr;///如果结点非空，可以提取结点内容
