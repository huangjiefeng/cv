...
void PhApiCallbacks::callProgress(OWPL_CALLSTATE_INFO * info) {
	if(info != NULL) {
		...
		HWND *video_hwnd = NULL;
		LOG_DEBUG("info->event"+info->event);
		/*********************/
		switch(info->event) {
			...
			case CALLSTATE_CONNECTED :
				switch(info->cause) {
					case CALLSTATE_CONNECTED_ACTIVE :
						...
						if (info->pData!=NULL&&strstr((char *)info->pData,"coolview"))
						{
							content = string((char *)info->pData);
							p->cvMessageReceivedEvent(*p,content,from);
						}
						else
			...
			case CALLSTATE_DISCONNECTED :
				switch(info->cause) {
					...
					case CALLSTATE_DISCONNECTED_NO_RESPONSE :
						if (info==NULL||!info->szRemoteIdentity) {
							from = "unknown@unknown.unknown";
						}
						else {
							from = info->szRemoteIdentity;
			...
			case CALLSTATE_VIDEO_EVENT :
				switch(info->cause) {
					...
					//		}
						if (info->pData)
						{
							p->videoFrameReceivedEvent(*p, info->hCall, (void *)info->pData, NULL);
						}
#endif
						break;
						/************************************************************************/
			...
			case CALLSTATE_IDENTITY_CHANGE :
				...
				break;
			/************************************************************************///Add by zhuang
			case CALLSTATE_NEWINFO :
				switch(info->cause) {
					case CALLSTATE_NEW_INFO_NORMAL :
						if (!info->szRemoteIdentity) {
							from = "unknown@unknown.unknown";
						}
						else {
	...
	switch(info->event) {
		...
		case LINESTATE_REGISTER_FAILED :
			switch(info->cause) {
				...
				case LINESTATE_CAUSE_NOT_PERMIT:
					p->setRegistered(false);
					p->phoneLineStateChangedEvent(*p, info->hLine, EnumPhoneLineState::PhoneLineStateNotPermitted);
					p->disconnectedEvent(*p, true, "Not permitted by sip server");
					break;
...
void PhApiCallbacks::messageProgress(OWPL_MESSAGE_INFO * info) {
	...
	if ((ctype == "application"||ctype == "Application") && subtype == "coolview") 
	{
		p->cvMessageReceivedEvent(*p,content,from);
		return;
	}
	...
	case MESSAGE_NEW : {
		...
		if (ctype == "typingstate") {
			...
			p->typingStateChangedEvent(*p, *imChatSession, from, state);
		} else if (ctype == "application" && subtype == "coolview") 
		{
			p->cvMessageReceivedEvent(*p,content,from);
		}
		else {
...
void PhApiCallbacks::onNotify(OWPL_NOTIFICATION_INFO * info) {
	...
	switch(info->event) {
		...
		case NOTIFICATION_UNKNOWN :
			content = std::string(info->szXmlContent); 
			if (strstr(info->szXmlContent,"coolview")!=NULL)
			{
				p->cvMessageReceivedEvent(*p,content,buddy);
			}
