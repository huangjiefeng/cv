...
public:
	...
	int addVirtualLine(const std::string & displayName,
		...
		const std::string & proxyServer, const std::string & registerServer,const int address_family);
	...
	* @param mime 内容对应的类型
	*/
	int registerVirtualLine(int lineId , const char* content=0 , const char* mime=0 );
	...
	int makeCall2(int lineId, const std::string & sipAddress, bool enableVideo,int width,int height,int bitrate,const char *codecSerial);
	/**************************************************************************************************************/
	...
	void acceptCall2(int callId, bool enableVideo,int width,int height,int bitrate,const char *codecSerial);
	/************************************************************************/
	/************************************************************************/
	/*Start Conference*/
	void startConference(int lineId,const std::string & confURI,const std::string & szContent);
	int makeJoinConfInvite(int lineId,const std::string & focusURI,const std::string & szContent, const MediaInfo &mediaInfo);
	...
	void unsubscribeTo(const std::string & contactId);
	//video窗口resize
	void resizeVideoWindow(const std::string &userId,const HWND &remoteHwnd,int fullscreenflag);


	static void rtpStatRecvHandler( void * pclass, const RtpStatItem *rtpstat );

	//接收到一个传输统计，进行处理（由下层回调）
	static void tranStatRecvHandler(void * pClass, const TranStat * transtat);
	...
	int makeInfoCall(int callId,char * szContent);
	
	//指定编码用的宽和高
	void setVideoWidthAndHeight(const int&width, const int &height);
	 ...
	 */
	void setCrossBar( const std::string& crossbarName , const std::string& crossbarInputType );
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////
	void sendQosPara(int lineId, const std::string & focusURI, const std::string & szContent);
	 ...
	 */
	 ...
	 int sendSipMessage(int lineId, string remoteURI, string msgContent);
