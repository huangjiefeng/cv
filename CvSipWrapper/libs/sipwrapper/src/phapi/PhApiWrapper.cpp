...
PhApiWrapper::PhApiWrapper(PhApiCallbacks & callbacks)
	...
	//phapi_media_set_transtatcallback(this,tranStatRecvHandler);
...
int PhApiWrapper::addVirtualLine(const std::string & displayName,
	...
	if (_isInitialized) {
		...
		if (owplLineAdd(displayName.c_str(), identity.c_str(), registerServer.c_str(), tmp.c_str(), OWPL_TRANSPORT_UDP, REGISTER_TIMEOUT, &hLine,address_family) != OWPL_RESULT_SUCCESS) {
...
int PhApiWrapper::registerVirtualLine(int lineId,const char* content , const char* mime) {
	phoneLineStateChangedEvent(*this, lineId, EnumPhoneLineState::PhoneLineStateProgress);
	int result = owplLineRegister(lineId, 1, content , mime );
	if (result != 0) {
...
//重载此函数，传入width，height，bitrate
int PhApiWrapper::makeCall2(int lineId, const std::string & sipAddress, bool enableVideo,int width,int height,int bitrate,const char *codecSerial) {
	OWPL_CALL hCall;
	...
	if(owplCallConnect2(hCall, sipAddress.c_str(), mediaFlags,width,height,bitrate,codecSerial) != OWPL_RESULT_SUCCESS) {
		return -1;
	}
...
//重载此函数，传入width，height，bitrate
void PhApiWrapper::acceptCall2(int callId, bool enableVideo,int width,int height,int bitrate,const char *codecSerial) {
	int mediaFlags = VIDEO_RX_ONLY_FLAGS;
	if (enableVideo) {
		mediaFlags = VIDEO_FLAGS;
	}

	owplCallAnswer2(callId, mediaFlags,width,height,bitrate,codecSerial);
}
/************************************************************************/
...
void PhApiWrapper::playDtmf(int callId, char dtmf) {
	phSendDtmf(callId, dtmf, 3);		//相关的函数已经取消，zhenHua.sun 2010-06-29
}

void PhApiWrapper::playSoundFile(int callId, const std::string & soundFile) {
	phSendSoundFile(callId, soundFile.c_str());	//无用，注释掉  zhenHua.sun  2010-06-28
}
...
bool PhApiWrapper::setCallInputAudioDevice(const AudioDevice & device) {
	...
	std::string deviceId = _inputAudioDevice.getName();
	strncpy(phcfg.audio_config.audio_input_device, deviceId.c_str(), sizeof(phcfg.audio_config.audio_input_device));
...
bool PhApiWrapper::setCallOutputAudioDevice(const AudioDevice & device) {
	...
	std::string deviceId = _outputAudioDevice.getName();
	strncpy(phcfg.audio_config.audio_output_device, deviceId.c_str(), sizeof(phcfg.audio_config.audio_output_device));
	return setAudioDevices();
}
...
void PhApiWrapper::joinConference(int confId, int callId) {
	...
	//phConf函数体已被取消 zhenHua.sun  2010-06-28
...
void PhApiWrapper::splitConference(int confId, int callId) {
	...
	//以下函数已经被取消，注释掉   zhenHua.sun 2010-06-28
	if (callId1 != SipWrapper::CallIdError && callId2 != SipWrapper::CallIdError) {
		phStopConf(callId1, callId2);
		callId1 = SipWrapper::CallIdError;
		callId2 = SipWrapper::CallIdError;
...
void PhApiWrapper::init(int ipv6_enable) {
	setNetworkParameter();
	...
	*/
	...
	if(owplInit(1, _sipLocalPort, _sipLocalPort, _sipLocalPort + 1, NULL, 0,ipv6_enable) == OWPL_RESULT_SUCCESS) {
		_isInitialized = true;
...
#ifdef ENABLE_VIDEO
	phVideoControlSetCameraFlip((int)flip);		//无用，注释掉 zhenHua.sun　2010-06-29
#endif
}
...
/************************************************************************/
/*Start Conference*/
void PhApiWrapper::startConference(int lineId,const std::string & confURI,const std::string & szContent)
{
	int messageId;
	owplConfMsgSend(lineId, confURI.c_str(), szContent.c_str(), "application/coolview", &messageId);
}

int PhApiWrapper::makeJoinConfInvite(int lineId,const std::string & focusURI,const std::string & szContent, const MediaInfo &mediaInfo)
{
	OWPL_CALL hCall;
	if(owplCallCreate(lineId, &hCall) != OWPL_RESULT_SUCCESS) {
		return -1;
	}
	owplMakeJoinConfInvite(hCall, focusURI.c_str(), szContent.c_str(), "Application/coolview", mediaInfo);
	return hCall;
}

int PhApiWrapper::media_send_start(char * AudioIP,int AudioPort,char * VideoIP,int VideoPort,HWND previewHwnd,int videoWidth,int videoHeight)
{
	//int result = phapi_media_send_start(AudioIP,AudioPort,VideoIP,VideoPort,previewHwnd,videoWidth,videoHeight);
	//SendGraphInfo sendInfo;
	//sendInfo.initial( phcfg.video_config.video_device , phcfg.audio_config.audio_input_device , 
	//	phcfg.audio_config.audio_output_device, AudioIP , AudioPort , "SPEEX" , 16000, 16 , 1,
	//	VideoIP , VideoPort , 0 , videoWidth , videoHeight , phcfg.video_config.crossbar_name , phcfg.video_config.crossbar_input_type);
	//
	//QByteArray input_bytes;
	//QDataStream out(&input_bytes , QIODevice::WriteOnly );
	//out << sendInfo;
	//CvStreamedMediaClient::getInstance()->createSendGraph( input_bytes );

	//phapi_media_set_send_rtpstatcallback(this,rtpStatRecvHandler);
	return 0;
}

void PhApiWrapper::rtpStatRecvHandler( void * pclass, const RtpStatItem *rtpstat )
{
	if (rtpstat == NULL)
		return;
	PhApiWrapper *p = static_cast<PhApiWrapper *>(pclass);
	if (p)
		p->rtpStatRecvEvent(*p, *rtpstat); //引发事件
}

void PhApiWrapper::media_send_stop(char * AudioIP,int AudioPort,char * VideoIP,int VideoPort)
{
	////phapi_media_send_stop(AudioIP,AudioPort,VideoIP,VideoPort);

	//NetInfo info;
	//info.audio_port = AudioPort;
	//info.video_port = VideoPort;
	//info.ip_addr = QString::fromLocal8Bit( VideoIP );
	//QByteArray input_bytes;
	//QDataStream out( &input_bytes , QIODevice::WriteOnly );
	//out << info;
	//CvStreamedMediaClient::getInstance()->stopSend( input_bytes );
}

int PhApiWrapper::media_recv_start(std::string userId,char * AudioIP,int AudioPort,const char* audioCodecMIME , const int audioSampleRate , const int audioBps , 
								   const int audioChannels,char * VideoIP,int VideoPort,char *codecMIME,HWND remoteHwnd,int videoWidth,int videoHeight)
{
	//return phapi_media_recv_start(userId.c_str(),AudioIP,AudioPort,audioCodecMIME , audioSampleRate , audioBps , audioChannels, 
		//VideoIP,VideoPort,codecMIME,remoteHwnd,videoWidth,videoHeight,this,rtpStatRecvHandler);

	////通过dbus创建接收窗口....zhenHua.sun 2010-10-24
	//RecvGraphInfo info;
	//info.initial( AudioIP , AudioPort , audioCodecMIME , audioSampleRate , audioBps ,
	//	audioChannels , VideoIP , VideoPort , 0 ,videoWidth , videoHeight, userId.c_str() );
	//QByteArray input_bytes;
	//QDataStream out(&input_bytes , QIODevice::WriteOnly );
	//out << info;
	//CvStreamedMediaClient::getInstance()->createRecvGraph( QString::fromStdString(userId) , input_bytes );
	return 0;
}

void PhApiWrapper::media_recv_stop(std::string userId)
{
	////phapi_media_recv_stop(userId.c_str());
	//CvStreamedMediaClient::getInstance()->stopRecv( QString::fromStdString(userId));
}

//屏幕共享
int PhApiWrapper::screen_share_send_start(char * ScreenIP,int ScreenPort,int x,int y,int width,int height)
{
	return phapi_screen_share_send_start(ScreenIP,ScreenPort,x,y,width,height);
}
//取消桌面共享
void PhApiWrapper::screen_share_send_stop(char * ScreenIP,int ScreenPort)
{
	phapi_screen_share_send_stop(ScreenIP,ScreenPort);
}

//接收桌面共享
int PhApiWrapper::screen_share_recv_start(char * ScreenIP,int ScreenPort,HWND remoteHwnd,int videoWidth,int videoHeight)
{
	return phapi_screen_share_recv_start(ScreenIP,ScreenPort,remoteHwnd,videoWidth,videoHeight);
}
//取消接收桌面共享
void PhApiWrapper::screen_share_recv_stop()
{
	phapi_screen_share_recv_stop();
}

void PhApiWrapper::exit_conference(const std::string & contactId)
{
	//phapi_exit_conference();
	unsubscribeTo(contactId);

	//CoolView4.x以上不需要，但是换回CoolView3.x版本的话要回复
	////zhenHua.sun 2010-12-07
	//CvStreamedMediaClient::getInstance()->exitConference();
}

void PhApiWrapper::unsubscribeTo(const std::string & contactId) {
	std::string sipAddress = makeSipAddress(contactId);

	owplUnsubscribeFromUri(_wengoVline, sipAddress.c_str());
...
//video窗口resize
void PhApiWrapper::resizeVideoWindow(const std::string &userId,const HWND &remoteHwnd,int fullscreenflag)
{
	phapi_resize_video_window(userId.c_str(),remoteHwnd,fullscreenflag);
}

//禁止发言
void PhApiWrapper::forbidSpeak(char * AudioIP,int AudioPort)
{
	phapi_forbid_speak(AudioIP,AudioPort);
}

void PhApiWrapper::forbidSpeak(char * userId)
{
	phapi_forbid_speak2(userId);
}
//允许发言
void PhApiWrapper::permitSpeak(char * AudioIP,int AudioPort)
{
	phapi_permit_speak(AudioIP,AudioPort);
}

int PhApiWrapper::makeInfoCall(int callId,char * szContent)
{
	owplInfoCall(callId,szContent,"Application/coolview");
	return 0;
}

//指定编码用的宽和高
void PhApiWrapper::setVideoWidthAndHeight(const int&width, const int &height) 
{
	phcfg.video_config.video_webcam_capture_width = width;
	phcfg.video_config.video_webcam_capture_height = height;
}

void PhApiWrapper::sendQosPara(int lineId, const std::string & focusURI, const std::string & szContent)
{
	int messageId;
	owplConfMsgSend(lineId, focusURI.c_str(), szContent.c_str(), "application/coolview", &messageId);
}
...
*/
void PhApiWrapper::controlAudioSend( bool state )
{
	//phapi_control_audio( state );
}
/**
* @brief 控制视频的发送
* @param state 如果是true，则启动视频的发送，否则停止视频的发送
*/
void PhApiWrapper::controlVideoSend( bool state )
{
	//phapi_control_video( state );
}
//end
/************************************************************************/

//----------------------------------------------------------------------start add by zhenHua.sun 2010-07-17
/**
* @brief 设置与crossbar filter有关的数据。Crossbar指针对于视频采集卡。
* @param crossbarName 设备名称
* @param crossbarInputType 设备输入媒体类型
*/
void PhApiWrapper::setCrossBar( const std::string& crossbarName , const std::string& crossbarInputType )
{
	strncpy( phcfg.video_config.crossbar_name , crossbarName.c_str(), sizeof( phcfg.video_config.crossbar_name ) );
	strncpy( phcfg.video_config.crossbar_input_type , crossbarInputType.c_str() , sizeof( phcfg.video_config.crossbar_input_type) );
}

void PhApiWrapper::setDscp( const int dscp )
{
	////phapi_media_set_dscp(dscp);
	//QosDscpInfo info;
	//info.dscp = dscp;
	//QByteArray output_bytes;
	//QDataStream out( &output_bytes , QIODevice::WriteOnly );
	//out << info;
	//CvStreamedMediaClient::getInstance()->setQosInfo( SEND_STREAMED_MEDIA_ID , output_bytes );
}


void PhApiWrapper::tranStatRecvHandler( void * pClass, const TranStat * transtat )
{
	if (transtat == NULL)
		return;
	PhApiWrapper *p = static_cast<PhApiWrapper *>(pClass);
	if (p)
		p->tranStatRecvEvent(*transtat); //引发事件
}

int PhApiWrapper::sendSipMessage( int lineId, string remoteURI, string msgContent )
{
	int messageId;
	owplConfMsgSend(lineId, remoteURI.c_str(), msgContent.c_str(), "application/coolview", &messageId);
	return messageId;
}
