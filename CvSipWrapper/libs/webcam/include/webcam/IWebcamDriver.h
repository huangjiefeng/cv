...
public:
	...
	virtual void getCodecParameter(const std::string & deviceName,int &width,int &height,int &bitrate,std::string & codecSerial) = 0;
	/************************************************************************/
	 ...
	 * @return CrossBar设备列表
	 */
	virtual StringList getCrossbarDeviceList( )=0;
	/**
	 * @brief 用于获取指定crossbar的输入类型
	 * @return crossbar支持的媒体类型
	 */
	virtual StringList getCrossbarInputType( const std::string &crossbarName )=0;
	//*************************************
	 ...
	 * @brief 获取当前设备支持的媒体类型
	 */
	virtual StringList getVideoDeviceMediaType( const std::string &deviceName ) =0;
};
