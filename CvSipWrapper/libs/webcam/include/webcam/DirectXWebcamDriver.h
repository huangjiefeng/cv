...
public:
	...
	void getCodecParameter(const std::string & deviceName,int &width,int &height,int &bitrate,std::string & codecSerial);
	/************************************************************************/
	 ...
	 * @return 设备列表
	 */
	StringList getCrossbarDeviceList( );

	/**
	 * @brief 用于获取指定crossbar的输入类型
	 * @return crossbar支持的媒体类型
	 */
	StringList getCrossbarInputType( const std::string& crossbarName );
	//*************************************
	 ...
	 * @brief 获取当前设备支持的媒体类型
	 */
	StringList getVideoDeviceMediaType( const std::string &deviceName );
