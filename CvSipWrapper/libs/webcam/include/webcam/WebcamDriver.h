...
public:
	...
	WEBCAM_API void getCodecParameter(const std::string & deviceName,int &width,int &height,int &bitrate,std::string & codecSerial);
	/************************************************************************/
	 ...
	 */
	WEBCAM_API StringList getCrossbarDeviceList( );

	/**
	 * @brief 用于获取指定crossbar的输入类型
	 * @return crossbar支持的媒体类型
	 */
	WEBCAM_API StringList getCrossbarInputType( const std::string& crossbarName );
	//*************************************
	/** 
	 * zhenHua.sun 2010-10-20
	 * @brief 获取当前设备支持的媒体类型
	 */
	WEBCAM_API StringList getVideoDeviceMediaType( const std::string &deviceName );
