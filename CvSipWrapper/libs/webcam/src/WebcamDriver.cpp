...
void WebcamDriver::getCodecParameter(const std::string & deviceName,int &width,int &height,int &bitrate,std::string & codecSerial)
{
	_webcamPrivate->getCodecParameter(deviceName,width,height,bitrate,codecSerial);
}
/************************************************************************/
...
*/
StringList WebcamDriver::getCrossbarDeviceList( )
{
	RecursiveMutex::ScopedLock lock(_mutex);

	return _webcamPrivate->getCrossbarDeviceList();
...
*/
StringList WebcamDriver::getCrossbarInputType( const std::string& crossbarName )
{
	RecursiveMutex::ScopedLock lock(_mutex);

	return _webcamPrivate->getCrossbarInputType( crossbarName );
}

StringList WebcamDriver::getVideoDeviceMediaType( const std::string &deviceName )
{
	RecursiveMutex::ScopedLock lock(_mutex);

	return _webcamPrivate->getVideoDeviceMediaType( deviceName );
}
