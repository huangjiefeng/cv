...
extern "C"
{
    ...
    typedef struct _RtpStatItem
    {
        ...
        int rem_port;       /*远程主机端口*/
        char local_addr[MAX_IP_STRING_LENGTH];  /*RTP传输的本地IP地址，该值一般是实际IP地址，但也有可能是回环地址或零地址; 组播传输时该值是IP组播地址*/
    ...
    static inline  void rtpStatItem_init(RtpStatItem* rtpstat)
    {
        ...
        //memset(rtpstat->remaddr, 0, MAX_IP_STRING_LENGTH);
        rtpstat->rem_addr_len = 0;
        rtpstat->rem_port = 0;
        //memset(rtpstat->localaddr, 0, MAX_IP_STRING_LENGTH);
	...
	typedef struct rtp_stat_information
	{	
		int		graph_id;	   //graph的id，发送graph可以设置为0，接收graph的id大于0
		RtpStatItem rtpstat;
	}RTP_STAT_INFO;
