...
{
	/************************************************************************///Add by zhuang 09-04-10
	/*修改使得每次读取的是一个完整的sip消息*/
	//int readIndex = * source_read_front_index;
	//char *temp;
	//char *temp2;
	//char *header;
	//char contentLengthBuf[10];
	//char *headerBuf;
	//int content_length;
	//int headerLength;
	//memset(contentLengthBuf,0,10);
	//owqueue_read_circular_buffer_without_commit (source, source_size, * source_read_front_index, destination, length) ;
	//header = (char *)destination;
	//if (strstr(header,"SIP/2.0")!=NULL)
	//{
	//	header = strstr(header,"\r\n\r\n");
	//	header +=strlen("\r\n\r\n");
	//	if(header!=NULL)
	//	{
	//		headerLength = header-destination;
	//		headerBuf = malloc(headerLength+1);
	//		memset(headerBuf,0,headerLength+1);
	//		strncpy(headerBuf,destination,headerLength);
	//		temp = strstr(headerBuf,"Content-Length:");//找到Content-Length
	//		if (temp!=NULL)
	//		{
	...
	//				temp2++;
	//			}
	//			strncpy(contentLengthBuf,temp,temp2-temp);
	//			content_length = atoi(contentLengthBuf);
	//			if (content_length>0)
	//			{
	//				memset(destination,0,length);
	//				owqueue_read_circular_buffer_without_commit (source, source_size, * source_read_front_index, destination, headerLength+content_length) ;
	//			}
	//			length = headerLength+content_length;
	//		}
	//		free(headerBuf);
	//	}
	//}
	...
	//readIndex = * source_read_front_index;
	/************************************************************************/
	owqueue_read_circular_buffer_without_commit (source, source_size, * source_read_front_index, destination, length) ;
	owqueue_shift_circular_buffer_index (source_read_front_index, length, source_size) ;
	return ;
...
/* TODO: improve MULTI_PACKET by reading many packets at once */
	...
	/************************************************************************///Add by zhuang 09-04-10
	/*                                                                     
	length = (local_data_buffer_read_front_index>queue->data_buffer_read_front_index)?local_data_buffer_read_front_index-queue->data_buffer_read_front_index:local_data_buffer_read_front_index+queue->data_buffer_size-queue->data_buffer_read_front_index; */
	/************************************************************************/
...
int
cvqueue_read_whole_sip_message
(
 OWQueue * queue,
 void * destination
 )
{
	...
	int headerLength = 0;
	void *source = queue->data_buffer;
	int source_size = queue->data_buffer_size;
	int length = owqueue_get_data_length(queue);
	int *source_read_front_index = &queue->data_buffer_read_front_index;
	...
	if (strstr(header,"SIP/2.0")!=NULL)
	{
		...
		if(header!=NULL)
		{
			...
			if (temp!=NULL)
			{
				...
				strncpy(contentLengthBuf,temp,temp2-temp);
				if(contentLengthBuf!=NULL)
					content_length = atoi(contentLengthBuf);
				if (content_length>0)
				{
					if (length<headerLength+content_length)
					{
						free(headerBuf);
						return -1;
					}
