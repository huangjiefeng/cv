...
#define PACKET_MAXIMUN 10

static OWQueue * cvSip_tcp_data_queue = NULL ;   /* in blocking mode */
...
)
{
	...
	char tempBuf[OWSL_TCP_PACKET_AVG_SIZE*PACKET_MAXIMUN];
	memset(tempBuf,0,OWSL_TCP_PACKET_AVG_SIZE*PACKET_MAXIMUN);
	/* get packet and packet size */
	received_bytes = recv (socket->system_socket, buffer, OWSL_TCP_BUFFER_SIZE, MSG_PEEK) ;
	/************************************************************************/
	/*先将数据存入自定义的缓冲区，取出一条完整的消息再执行*/
	if (received_bytes > 0)
	{
		written_bytes = owqueue_write (cvSip_tcp_data_queue, buffer, received_bytes, NULL, OWQUEUE_NON_BLOCKING | OWQUEUE_TRANSACTION_OPEN) ;
		owqueue_write_transaction_close (cvSip_tcp_data_queue, received_bytes == written_bytes ? received_bytes : 0) ;
		if (written_bytes>0)
		{
			received_bytes = recv (socket->system_socket, buffer, written_bytes, 0) ;
			received_bytes = cvqueue_read_whole_sip_message(cvSip_tcp_data_queue,tempBuf);
			while (received_bytes>0)
			{
				written_bytes = owqueue_write (socket->in_queue, tempBuf, received_bytes, NULL, OWQUEUE_NON_BLOCKING | OWQUEUE_TRANSACTION_OPEN) ;
...
(void)
{
	...
	/************************************************************************/
	/*初始化自定义队列缓冲区*/
	cvSip_tcp_data_queue = owqueue_new
		(
		OWSL_TCP_PACKET_AVG_SIZE*PACKET_MAXIMUN,										/* usable size */
		OWQUEUE_NO_PACKET,                                                   /* mode */
		PACKET_MAXIMUN,																	 /* packet maximum */
		0																	 /* packet info size */
		) ;
	/************************************************************************/
