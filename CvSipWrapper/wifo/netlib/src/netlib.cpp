...
)
{
	...
	_family = (OWSLAddressFamily)((OWSLAddress *) bound_address)->ss_family ;
	if(_family==AF_INET)
	{
		sprintf
			(
...
)
{
	...
	//支持IPv6
	//OWSLAddressFamily address_family = OWSL_AF_IPV6;
	//OWSLAddressFamily address_family = OWSL_AF_IPV4;
	//if (owsl_address_public_ip_get (OWSL_AF_IPV4, public_ip, sizeof (public_ip)) == 0)
	OWSLAddressFamily address_family = (OWSLAddressFamily)addressfamily;
	if (owsl_address_public_ip_get2 (address_family, public_ip, sizeof (public_ip)) == 0) //by zhuang 0625
	{
		ip = public_ip ;
	}

	/* TODO: IPv6 support in netlib */
	//添加对IPv6的支持//by zhuang 0625
	//udp_socket = owsl_socket_by_type (OWSL_TYPE_IPV4_UDP) ;
	if(address_family == OWSL_AF_IPV4)
		udp_socket = owsl_socket_by_type (OWSL_TYPE_IPV4_UDP) ;
	else
		udp_socket = owsl_socket_by_type (OWSL_TYPE_IPV6_UDP) ;
	if (udp_socket <= 0)
	{
		return NETLIB_FALSE ;
	}
	...
	/* TODO: IPv6 support in netlib */
	local_address = owsl_address_ip_port_new (address_family, ip, local_port) ;
	...
	if(address_family == OWSL_AF_IPV4)
		...
		return_code = owsl_bind (udp_socket, local_address, /*OWSL_ADDRESS_SIZE*/sizeof(struct sockaddr_in6)) ;
	free (local_address) ;
	if (return_code != 0)
	{
		owsl_close (udp_socket) ;
	...
	remote_address = owsl_address_port_new (address_family, sip_server, sip_port) ;
	...
	if(address_family == OWSL_AF_IPV4)
		...
		return_code = owsl_connect (udp_socket, remote_address, /*OWSL_ADDRESS_SIZE*/sizeof(struct sockaddr_in6)) ;
	free (remote_address) ;
	if (return_code != 0)
	{
