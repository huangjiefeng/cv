...
)
{
	...
	*/
	...
	return_code = eXosip_init(0, 0, udpPort, tcpPort, tlsPort,ipv6_enable);
	if (return_code != 0)
	{
...
{	
	...
	if (vl) {
		...
		*phLine = ph_vline2vlid(vl);
		 ...
		 */
		ret = phAddVline2(displayname, sipUsername, sipServer, sipProxy, sipTransport, 0,address_family); 
		if (ret < 0) {
			return OWPL_RESULT_FAILURE;
		}
 ...
 * @brief 2011-06-15添加了两个新的参数，分别是content和mime
 * @param content 注册消息所带的内容
 * @param mime 内容对应的类型
 ...
 */
OWPL_RESULT
owplLineRegister(const OWPL_LINE hLine, const int bRegister,const char* content , const char*mime )
{
	int ret;
	if (bRegister) {
		ret = phvlRegister(hLine, content , mime);
	}
	else {
...
OWPL_RESULT
owplCallConnect2(const OWPL_CALL hCall,
				const char* szAddress,
				int mediaStreams,
				int width,int height,int bitrate,const char *codecSerial)
{
	phcall_t *ca = ph_locate_call_by_cid(hCall);

	reset_video_payload(codecSerial);
	if (phLinePlaceCall_withCa2(ca->vlid, szAddress, 0, 0, mediaStreams, ca,width,height,bitrate) <= 0)
	{
		return OWPL_RESULT_FAILURE;
	}
...
/*重载                                                                  */
OWPL_RESULT owplCallAnswer2(const OWPL_CALL hCall, int mediaStreams,int width,int height,int bitrate,const char *codecSerial)
{
	int ret;
	reset_video_payload(codecSerial);
	ret = phAcceptCall3_2(hCall, 0, mediaStreams,width,height,bitrate);
...
/*会议相关消息*/
OWPL_RESULT
owplConfMsgSend(OWPL_LINE hLine,
...
OWPL_RESULT 
owplMakeJoinConfInvite(const OWPL_CALL hCall,
							   const char * szRemoteUri,
							   const char * szContent,
							   const char * szMIME,
							   const struct MediaInfo mediaInfo)
{
	int i;
	osip_message_t *invite;
	char *proxy ;
  ...
  // video sdp
	reset_video_payload(mediaInfo.video_codec);
	set_video_width_height(mediaInfo.video_width, mediaInfo.video_height, mediaInfo.video_fps);
  //audio sdp
  reset_audio_payload(mediaInfo.audio_codec, mediaInfo.audio_rate, mediaInfo.audio_bits, mediaInfo.audio_channel);
	...
	if((i = eXosip_build_initial_invite(&invite, szRemoteUri, from, proxy, "")) != 0){
		return -1;
	}

	eXosip_lock();

	i = eXosip_initiate_call_with_body(account, invite, szMIME, szContent, 0);
	if (i <= 0) {
		return OWPL_RESULT_FAILURE;
	}
...
//发送Info消息
OWPL_RESULT owplInfoCall(const OWPL_CALL hCall,char * szContent,char * szMIME)
{
	phcall_t *ca = ph_locate_call_by_cid(hCall);
	if (ca == NULL)
	{
	...
	eXosip_lock();
	eXosip_info_call(ca->did, szMIME, szContent);
	eXosip_unlock ();
}
int phapi_media_send_start(char * AudioIP,int AudioPort,char * VideoIP,int VideoPort,HWND previewHwnd,void *pClass,void *func,int videoWidth,int videoHeight)
{
	return ph_media_send_start(AudioIP,AudioPort,VideoIP,VideoPort,previewHwnd,pClass,func,videoWidth,videoHeight);
}

void phapi_media_send_stop(char * AudioIP,int AudioPort,char * VideoIP,int VideoPort)
{
	ph_media_send_stop(AudioIP,AudioPort,VideoIP,VideoPort);
}

int phapi_media_recv_start(const char *userId,char * AudioIP,int AudioPort,char * VideoIP,int VideoPort,char *codecMIME,HWND remoteHwnd,int videoWidth,int videoHeight,void *pClass,void *func)
{
	return ph_media_recv_start(userId,AudioIP,AudioPort,VideoIP,VideoPort,codecMIME,remoteHwnd,videoWidth,videoHeight,pClass,func);
}

void phapi_media_recv_stop(const char *userId)
{
	ph_media_recv_stop(userId);
}

void phapi_exit_conference()
{
	ph_exit_conference();
}

//屏幕共享
 int phapi_screen_share_send_start(char * ScreenIP,int ScreenPort,int x,int y,int width,int height)
 {
	return ph_screen_share_send_start(ScreenIP,ScreenPort,x,y,width,height);
 }
//取消桌面共享
void phapi_screen_share_send_stop(char * ScreenIP,int ScreenPort)
{
	ph_screen_share_send_stop(ScreenIP,ScreenPort);
}
//接收桌面共享
int phapi_screen_share_recv_start(char * ScreenIP,int ScreenPort,HWND remoteHwnd,int videoWidth,int videoHeight)
{
	return ph_screen_share_recv_start(ScreenIP,ScreenPort,remoteHwnd,videoWidth,videoHeight);
}
//取消接收桌面共享
void phapi_screen_share_recv_stop()
{
	ph_screen_share_recv_stop();
}

//禁止发言
void phapi_forbid_speak(char * AudioIP,int AudioPort)
{
	ph_forbid_speak(AudioIP,AudioPort);
}

void phapi_forbid_speak2(char * userId)
{
	ph_forbid_speak2(userId);
}
//允许发言
void phapi_permit_speak(char * AudioIP,int AudioPort)
{
	ph_permit_speak(AudioIP,AudioPort);
}

//video窗口resize
void phapi_resize_video_window(const char *userId,const HWND remoteHwnd,int fullscreenflag)
{
	ph_resize_video_window(userId,remoteHwnd,fullscreenflag);
}

OWPL_RESULT
owplUnsubscribeFromUri(OWPL_LINE  hLine, const char * szRemoteUri) 
{
	OWPL_SUB hSub;
	int i = 0;
