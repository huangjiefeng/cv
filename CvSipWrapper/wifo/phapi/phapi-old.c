...
//#include "phcodec.h"       编解码，无用  zheHua.sun 2010-06-28
#include "mxcodec.h"		//自定义的编解码参数  zhenHua.sun 2010-06-28
#include <pthread.h>
...
/************************************************************************///Add by zhuang 08-12-19
void get_fmtp_from_je(phcall_t *ca,eXosip_event_t *je,const char* avflag)
{
	int payloadIndex;
	int fmtpModifIndex;
	const char* format_modifier = NULL;
	...
	if(strcmp(avflag,"video")==0)
		pnum = je->video_payload;
	else
		pnum = je->payload;
	vpayload = malloc(50);
	snprintf(vpayload, 50, "%d",pnum);
	for (fmtpModifIndex = 0;!osip_list_eol(je->format_modifiers, fmtpModifIndex); fmtpModifIndex++)
	{
		format_modifier = (const char*)osip_list_get(je->format_modifiers, fmtpModifIndex);

		if ((pnum <10 &&
			0==osip_strncasecmp(format_modifier, vpayload, 1))
			||(pnum>9 && pnum<100 &&
			0==osip_strncasecmp(format_modifier, vpayload, 2))
			||(pnum >100 && pnum<128 &&
			0==osip_strncasecmp(format_modifier, vpayload, 3)))
		{
			strcpy(ca->video_fmtp,format_modifier);
		}
	}
}
/************************************************************************/
...
/*重载此函数，获取video的窗口句柄                                       */
void ph_frame_display_cbk2(void *ctx, void *event)
{
	phcall_t *ca = (phcall_t *) ctx;
...
phLinePlaceCall_withCa2(int vlid, const char *uri, void *userdata, int rcid, int streams, phcall_t *ca0,int width,int height,int bitrate)
{
	int i;
	osip_message_t *invite;
	phcall_t *ca = 0;
	...
	eXosip_lock();
	DBG_SIP_NEGO("NO STUN ports (a.local=%s, a.public=%s) (v.local=%s, v.public=%s)\n",local_voice_port,0,local_video_port,0);
	i = eXosip_initiate_call2(vl->sipAccount, invite, userdata, NULL, local_voice_port, optional(local_video_port),  0, 0,width,height,bitrate);
...
/*重载此函数,"_2"表示对原来函数的重载，传入视频帧的width，height，bitrate等参数*/
int phAcceptCall3_2(int cid, void *userData, int streams,int width,int height,int bitrate)
{
	int i;
	phcall_t *ca = ph_locate_call_by_cid(cid);
	phCallStateInfo_t info;
	...
	eXosip_lock();
	DBG_SIP_NEGO("NO STUN ports (a.local=%s, a.public=%s) (v.local=%s, v.public=%s)\n",local_voice_port,0,local_video_port,0);
	i = eXosip_answer_call2(ca->did, 200, local_voice_port, optional(local_video_port), 0, 0,width,height,bitrate);
...
{
	/** Dtmf暂时无用，注释掉......zhenHua.sun 2010-06-25
	phcall_t *ca = ph_locate_call_by_cid(cid);
...
{
	/** 该函数目前无用，先注释掉...zhenHua.sun 2010-06-25
	phcall_t *ca = ph_locate_call_by_cid(cid);
...
{
	return phAddVline2(NULL, username, server, proxy, transport, regTimeout,1);
}
...
int
phAddVline2(const char *displayname, const char* username, const char *server, const char*  proxy, OWPL_TRANSPORT_PROTOCOL transport, int regTimeout,int address_family)
{
	phVLine *vl;
	int oldTimeout = 0;
	char srvbuf[256];
...
/************************************************************************/
/*从config文件中得到codec的排列，并添加到sdp*/
void setup_video_payload_by_codec_serial(const char *codec_serial)
{
	/* parse the codec serial */
	char *p=NULL,*temp=NULL,*codec=NULL;
	char *codec_mime;
	codec_mime = (char *)malloc(15);
	temp = codec_serial;
	p = strchr(temp,'/');
	while (p!=NULL)
	{
		memset(codec_mime,0,15);
		strncpy(codec_mime,temp,p-temp);
		codec_mime = strcat(codec_mime,"/90000");
		setup_video_payload(codec_mime);
		temp = p+1;
		p = strchr(temp,'/');
	}
	memset(codec_mime,0,15);
	strcpy(codec_mime,temp);
	codec_mime = strcat(codec_mime,"/90000");
	setup_video_payload(codec_mime);
	free(codec_mime);
}
...
//重设sdp中video的序列
void reset_video_payload(const char *codec_serial)
{
	eXosip_sdp_negotiation_remove_video_payloads();
	setup_video_payload_by_codec_serial(codec_serial);
}

//设置sdp中的width和height
void set_video_width_height(const int width,const int height , const int fps)
{
	eXosip_sdp_negotiation_set_video_width_height(width,height ,fps);
}

void setup_audio_payload_by_codec(const char *codec, int rate, int bits, int channel)
{
  char  tmp[64];
  char  num[8];
  ph_media_payload_t  pt;
  ...
  if(!strncasecmp(codec, CODEC_SPEEX_MIME_STRING, sizeof(CODEC_SPEEX_MIME_STRING)))
  {
    snprintf(tmp, sizeof(tmp), "%s/%d/1", codec, rate);
  }
  else if(!strncasecmp(codec, CODEC_AACLC_MIME_STRING, sizeof(CODEC_AACLC_MIME_STRING)))
  {
    snprintf(tmp, sizeof(tmp), "%s/%d/1", codec, 16000); // 保持与phmedia.c中定义的默认值一致
  }

  if (ph_media_supported_payload(&pt, tmp))
  {
    DBG_CODEC_LOOKUP("...setup accepted : %d - %s/%d\n", pt.number, pt.string, rate);
    snprintf(num, sizeof(num), "%d", pt.number);
    snprintf(tmp, sizeof(tmp), "%d %s/%d/%d", pt.number, pt.string, rate, channel);
...
//重设sdp中audio的序列
void reset_audio_payload(const char *codec, int rate, int bits, int channel)
{
	eXosip_sdp_negotiation_remove_audio_payloads();
	setup_audio_payload_by_codec(codec, rate, bits, channel);
}
...
ph_payloads_init()
{
	...
	//加入MPEG-2 payload
	setup_video_payload(CODEC_MPG2_MIME_STRING"/90000");
	//加入MPEG-4 payload
	setup_video_payload(CODEC_MPG4_MIME_STRING"/90000");
#if 0
...
ph_call_retrieve_payloads(phcall_t *ca, eXosip_event_t *je, int flags)
{
	...
	{
		/******************************************************************************************************************************///Add by zhuang 08-12-19
		//注释原来的语句，新加一个参数获得视频的fmtp，重载用到的函数
//		i = eXosip_retrieve_negotiated_video_payload(ca->did, &ca->video_payload, ca->video_payload_name, sizeof(ca->video_payload_name));
		i = eXosip_retrieve_negotiated_video_payload2(ca->did, &ca->video_payload,ca->video_fmtp ,ca->video_payload_name, sizeof(ca->video_payload_name));
		/********************************************************************************************************************************************************/
...
* this function is used everytime it is necessary to start media sessionsss
...
ph_call_media_start(phcall_t *ca, eXosip_event_t *je, int resumeflag)
{
	...
	//	s->frameDisplayCbk =  ph_frame_display_cbk;
		s->frameDisplayCbk =  ph_frame_display_cbk2;
		/************************************************************************/
		...
		/************************************************************************///Add by zhuang 08-12-19
		//获得视频的fmtp参数
		if(ca->video_fmtp!=NULL)
		{
			int length=strlen(ca->video_fmtp);
			if(msp->ipayloads[0].fmtp!=NULL)
				osip_free(msp->ipayloads[0].fmtp);
			msp->ipayloads[0].fmtp=osip_malloc(length+1);
			assert(msp->ipayloads[0].fmtp);
			strcpy(msp->ipayloads[0].fmtp,ca->video_fmtp);
		}
...
/*                                                                      */
void
ph_conference_call_answered(eXosip_event_t *je)
{
	phCallStateInfo_t info;
	phcall_t *ca, *rca=0;
...
void
ph_info_call(eXosip_event_t *je)
{
	phCallStateInfo_t info;
	phcall_t *ca, *rca=0;
...
void ph_reg_progress(eXosip_event_t *je)
{
	...
	else if (je->type == EXOSIP_REGISTRATION_FAILURE)
	{
		...
		{
			eXosip_lock();
			i = eXosip_register(je->rid, newtimeout,0,0);
			eXosip_unlock();
		...
		switch(je->status_code) {
			case 401:
				if( je->msg_body )
				{
					char* gb2312Msg = U2G( je->msg_body );
					if( gb2312Msg )
					{
						if( strstr(gb2312Msg, "密码错误")!=NULL ||
							strstr( gb2312Msg, "用户不存在")!=NULL )
						{
							_owplLineSetState(vlid, reg_event , LINESTATE_CAUSE_NOT_AUTHORIZED );
						}else
						{
							if( strstr(gb2312Msg,"该SIP账号不能在该终端上使用")!=NULL )
							{
								_owplLineSetState(vlid, reg_event , LINESTATE_CAUSE_NOT_PERMIT );
							}
						}
						free( gb2312Msg );
					}
...
void ph_notify_handler(eXosip_event_t *je)
{
	...
	if (strcmp(je->sip_event, "coolview") == 0)
	{
...
static int ph_process_call_event_default(eXosip_event_t * je) {
	...
	switch(je->type) {
		...
		case EXOSIP_CALL_ANSWERED:
			...
			if(strstr(je->msg_body,"coolview"))
				ph_conference_call_answered(je);
			else
				ph_call_answered(je);
			break;
...
#if 0
		time(&now);
		printf("%s================================<before eXosip_event_wait>==============================\n",ctime(&now));
#endif
		je = eXosip_event_wait(0,phWaitTimeout);
#if 0
		time(&now);
		if(je==NULL)
			printf("%s================================end eXosip_event_wait<NULL>==============================\n",ctime(&now));
		else
			printf("%s================================end eXosip_event_wait<%s>==============================\n",ctime(&now),evtnames[je->type]);
#endif
		if(je==NULL) {
			break;
		}
...
//===============================在外部文件定义的旧的接口函数将在这里进行保留==============

//+++++++++++原定义文件为phmedia-video-control.c
int phVideoControlSetCameraFlip(int flip) {
	//phcall_t *ca;
	//phvstream_t *sess;
	//phConfig_t *cfg = phGetConfig();
	phcfg.video_config.video_camera_flip_frame = flip;
	return 0;
}
