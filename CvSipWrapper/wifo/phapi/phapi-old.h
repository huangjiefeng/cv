...
MY_DLLEXPORT int phAddVline2(const char* displayname, const char* username, const char *host, const char*  proxy, OWPL_TRANSPORT_PROTOCOL transport, int regTimeout,int address_family);
 ...
 * @return             0 in case of success
 */
MY_DLLEXPORT int phPlaySoundFile(const char *fileName , int loop);		//函数体已被取消   ......zhenHua.sun 2010-06-25
 ...
 * @return             0 in case of success
 */
MY_DLLEXPORT int phSendSoundFile(int cid, const char *fileName);		//该函数目前已被取消...zhenHua.sun 2010-06-25
 ...
 *
 */
MY_DLLEXPORT int phConf(int cid1, int cid2);		//无用，注释掉  zhenHua.sun 2010-06-25
 ...
 *
 */
MY_DLLEXPORT int phStopConf(int cid1, int cid2);	//函数体已被取消  zhenHua.sun 2010-06-25
...
/**
 * zhenHua.sun 2010-10-19
 * @struct phAudioConfig
 */
struct phAudioConfig{
	int audio_channels;
	int audio_sample_rate;
	int audio_btis_per_sample;
	char audio_input_device[256];
...
#define PHAPI_VIDEO_LINE_AUTOMATIC	6
	...
	///用于支持高清的crossbar ...zhenHua.sun 2010-07-17
	char crossbar_name[256];		
	
	///记录选择的crossbar的输入类型...zhenHua.sun 2010-07-17
	char crossbar_input_type[36];
};
...
struct phConfig {
  ...
  struct phAudioConfig audio_config;		//用于存储audio的配置信息
};
#endif
