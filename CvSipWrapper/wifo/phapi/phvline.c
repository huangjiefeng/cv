...
int
phvlRegisterNoLock(int vlid,const char* content , const char* mime )
{
	...
	if (user && domain)
	{
		...
		if (vl->rid >= 0)
		{
			...
			ret = eXosip_register(vl->rid, vl->regTimeout,content , mime );
...
int
phvlRegister(int vlid, const char* content , const char*mime )
{
	int ret;

	eXosip_lock();
	ret = phvlRegisterNoLock(vlid, content , mime);
	eXosip_unlock();
...
phvlUnregister(int vlid)
{
	...
	if (vl) {
		...
		printf("\nError: Invalid call to phvlRegister\n");
		assert(0);
