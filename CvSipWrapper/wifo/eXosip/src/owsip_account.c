...
)
{
	...
	if (account_info != NULL)
	{
		...
		//添加IPv6/IPv4标记
		account_info->address_family = address_family;
		//
...
)
{
	...
	if (via != NULL)
	{
		...
		//在断网的时候用户信息会被清除，因此在此需要加多一个判断，防止空指针访问异常   ......zhenHua.sun 2010-12-06
		if( owsip_account_info_get(account)==NULL )
		{
			return NULL;
		}
		...
		(
			...
			(
				...
				/* TODO: use info->address_family */
				owsip_account_info_get (account)->address_family
				//address_family
			),
			ip_port,
			sizeof (ip_port)
...
)
{
	...
	account_info = owsip_account_info_new (account, name, user, domain, transport, proxy, port,address_family) ;
	if (account_info == NULL)
	{
		pthread_mutex_unlock (& mutex) ;
		pthread_mutex_destroy (& mutex) ;
