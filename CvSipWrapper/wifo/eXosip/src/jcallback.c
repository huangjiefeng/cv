...
static int cb_udp_snd_message(osip_transaction_t* tr, osip_message_t* sip, char* host, int port, int out_socket);
...
/************************************************************************/
/**/
static void cb_rcv2xx_4invite_coolview(osip_transaction_t* tr, osip_message_t* sip);
/************************************************************************/
 ...
 */
static int owsip_send(OWSIPAccount account, const void* buf, int len, int flags, const struct sockaddr* to, int tolen)
{
    ...
    {
        OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, "no matching SIP account found\n")) ;
        return -1 ;
    }
    eXosip_lock_jsocket();
    res = transport_sendto(transport, buf, len, flags, to, tolen);
...
int cb_snd_message(osip_transaction_t* tr, osip_message_t* sip, char* host, int port, int out_socket)
{
    int i;
    osip_via_t* via;
...
int cb_udp_snd_message(osip_transaction_t* tr, osip_message_t* sip, char* host, int port, int out_socket)
{
    ...
    address = owsl_address_port_new(eXosip.ip_family, tmp_host, (unsigned short) port) ;
    if(address == NULL)
    {
        osip_free(tmp_host);
        return -1 ;
    ...
    if(jinfo != NULL)
    {
        ...
        if(owsip_account_route_filter_is_enabled(account) && (!jinfo->jd || jinfo->jd->d_STATE == JD_AUTH_REQUIRED) && o_proxy && o_proxy->url && (strcmp(o_proxy->url->host,
                                                                                                                                                          tmp_host) == 0))
        {
    ...
    if(tr)
    {
        ...
        if(cid >= 0)
        {
            ...
            else/*    if (MSG_IS_RESPONSE_FOR(sip, "INVITE") && sip->status_code == 180)
...
static void cb_ict_kill_transaction(int type, osip_transaction_t* tr)
{
    ...
    if(i != 0)
    {
        OSIP_TRACE(osip_trace(__FILE__,
                              __LINE__,
                              OSIP_BUG,
                              NULL,
...
static void cb_nict_kill_transaction(int type, osip_transaction_t* tr)
{
    ...
    if(MSG_IS_REFER(tr->orig_request) && type == OSIP_NICT_KILL_TRANSACTION && tr->last_response == NULL)
    {
        eXosip_event_t* je;
    ...
    if(MSG_IS_NOTIFY(tr->orig_request) && type == OSIP_NICT_KILL_TRANSACTION && tr->last_response == NULL)
    {
        /* delete the dialog! */
    ...
    if(MSG_IS_NOTIFY(tr->orig_request) && type == OSIP_NICT_KILL_TRANSACTION && tr->last_response != NULL && tr->last_response->status_code > 299)
    {
        /* delete the dialog! */
        REMOVE_ELEMENT(eXosip.j_notifies, jn);
        eXosip_notify_free(jn);
    ...
    if(MSG_IS_NOTIFY(tr->orig_request) && type == OSIP_NICT_KILL_TRANSACTION && tr->last_response != NULL && tr->last_response->status_code > 199 &&
...
jinfo_t* __eXosip_new_jinfo(OWSIPAccount account, eXosip_call_t* jc, eXosip_dialog_t* jd, eXosip_subscribe_t* js, eXosip_notify_t* jn)
{
...
static void check_for_answer_with_sdp(eXosip_call_t* jc, osip_message_t* sip)
{
...
static void cb_rcv1xx(int type, osip_transaction_t* tr, osip_message_t* sip)
{
    ...
    if((MSG_IS_RESPONSE_FOR(sip, "INVITE") || MSG_IS_RESPONSE_FOR(sip, "SUBSCRIBE")))
    /*      && MSG_TEST_CODE(sip, 100)) */
    {
        ...
        {
            OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL, "eXosip: have eXosip_dialog with NULL osip_dialog\n"));
            return;
        }
    }
...
static void cb_rcv2xx_4invite_coolview(osip_transaction_t* tr, osip_message_t* sip)
{
    ...
    check_for_answer_with_sdp(jc, sip);
    {
        ...
        if(!osip_list_eol(&sip->bodies, 0)) /* need to build sdp answer */
        {
            char* body;
            char* body2;
            char* size;
            ...
            memset(ackresult, 0, 20);
            sip_body = (osip_body_t *)osip_list_get(&sip->bodies, 0);
            //收到login result
            if(strstr(sip_body->body, "LoginResult") != NULL)
            {
                if(strstr(sip_body->body, "success") != NULL)
                    strncpy(ackresult, "success", 20);
                else
                    strncpy(ackresult, "failure", 20);
                //  return;
            }
            ////得到video port
            //temp1 = strstr(sip_body->body,"<videoPort>");
            //temp1 +=strlen("<videoPort>");
            //temp2 = strstr(sip_body->body,"</videoPort>");
            //strncpy(vport,temp1,temp2-temp1);
            ////得到audio port
            //temp1 = strstr(sip_body->body,"<audioPort>");
            //temp1 +=strlen("<audioPort>");
            //temp2 = strstr(sip_body->body,"</audioPort>");
            //strncpy(aport,temp1,temp2-temp1);
            osip_negotiation_sdp_build_offer(eXosip.osip_negotiation, NULL, &sdp, aport, vport);
            i = sdp_message_to_str(sdp, &body);
            if(body == NULL)
            {
                return;
            }
            body2 = malloc(strlen(body) + 200);
            memset(body2, 0, sizeof(strlen(body) + 200));
            sprintf(body2, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><coolview  command=\"qos\"  type=\"LoginAck\"><result>%s</result><sdp>%s</sdp></coolview>", ackresult, body);
            i = osip_message_set_body(ack, body2, strlen(body2));
            if(i != 0)
            {
                return;
            }
            ...
            size = (char*)osip_malloc(6 * sizeof(char));
#ifdef __APPLE_CC__
            sprintf(size, "%li", strlen(body2));
#else
            sprintf(size, "%i", strlen(body2));
#endif
            osip_free(body);
            osip_free(body2);
            i = osip_message_set_content_length(ack, size);
            ...
            i = osip_message_set_content_type(ack, "application/coolview");
            if(i != 0)
            {
                return;
            }
        ...
        /*不需要发送suscribe了 2009-5-13
            //send subscribe
            sprintf(to,"sip:%s@%s",sip->to->url->username,sip->to->url->host);
            sprintf(from,"sip:%s@%s",sip->from->url->username,sip->from->url->host);
            sprintf(sip_route,"%s:%d",sip->from->url->host,port);
            //-1代表event是coolview
            eXosip_subscribe(1,to,from,sip_route,-1);
            */
    }
...
static void cb_rcv2xx(int type, osip_transaction_t* tr, osip_message_t* sip)
{
    ...
    {
        if(strcmp(sip->content_type->subtype, "coolview") == 0)
        {
            cb_rcv2xx_4invite_coolview(tr, sip);
        }
        else
...
static void cb_transport_error(int type, osip_transaction_t* tr, int error)
{
    ...
    if(MSG_IS_OPTIONS(tr->orig_request) && jc->c_dialogs == NULL && type == OSIP_NICT_TRANSPORT_ERROR)
    {
        /* delete the dialog! */
