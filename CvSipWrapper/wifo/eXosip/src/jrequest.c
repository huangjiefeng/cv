				 ...
				 char *proxy, int seqnum)
{
  ...
  */
  ...
  if (0==strcmp("INVITE", method_name) || 0==strcmp("SUBSCRIBE", method_name) || !strcmp("PUBLISH", method_name))
  {
	  osip_from_t *a_from;
	  int i;
 ...
 * @brief 根据制定的参数初始化注册消息体
 */
int  generating_register_with_message(osip_message_t **reg, char *from,
									  char *proxy, char *contact, int expires, char *route, int seqnum,const char*msg,const char*mime )
{

	int result = generating_register(reg , from , proxy , contact , expires , route , seqnum );
	if( result<0 )
		return result;

	if( msg!=0 && mime!=0 )
	{
		osip_message_set_body( *reg , msg , strlen(msg) );
		osip_message_set_content_type(*reg, mime);
	}
