  ...
  */
  ...
  if (sdp!=NULL)
	...
	else
	{
	  ...
	  eXosip_get_sdp_media_format_info(sdp, je);
  ...
  if (type==EXOSIP_IN_SUBSCRIPTION_NEW
      ...
      if (jd!=NULL&&jd->d_dialog!=NULL)
	{
	  ...
	  if (tr!=NULL && tr->orig_request!=NULL)
		...
		osip_message_header_get_byname(tr->orig_request, "event", 0, &event_hdr);
  ...
  if (je->format_modifiers)
    {
	  ...
	  while (!osip_list_eol(je->format_modifiers, pos))
      {
        format_modifier = (char*)osip_list_get(je->format_modifiers, pos);
	    osip_list_remove(je->format_modifiers, pos);
        osip_free(format_modifier);
      }
      
	  osip_free (je->format_modifiers);
      je->format_modifiers = NULL;
    }
  osip_free(je);
}
