			       ...
			       int status, osip_message_t *request)
{
  ...
  if (MSG_IS_SUBSCRIBE(request))
    {
      osip_header_t *exp;
      owsip_message_set_header(response, "Event", "presence");
...
complete_answer_that_establish_a_dialog2(osip_message_t *response, osip_message_t *request, char *ctct)
{
	...
	*/
	while (!osip_list_eol(&request->record_routes, pos))
	{
		...
		if (osip_record_route_clone(rr, &rr2))
		{
			return -1;
		}
	...
	if (ctct && ctct[0])
	{
		snprintf (contact, sizeof (contact), "%s", ctct) ;
	}
	else
	{
		if
		(
			...
			(
				owsip_account_get_from_received_request (request),
				contact,
				sizeof (contact)
			) == NULL
		)
...
generating_sdp_answer(osip_message_t *request, osip_negotiation_ctx_t *context)
{
  ...
  if (MSG_IS_INVITE(request)||MSG_IS_OPTIONS(request)||MSG_IS_RESPONSE_FOR(request, "INVITE"))
      ...
      if (i==200)
	{
	  ...
	  /************************************************************************/ 
	  //暂时不在这里销毁remote_sdp，因为后面还会用到，使用完再销毁
	  /*
...
/*                                                                      */
char *
generating_sdp_answer2(osip_message_t *request, osip_negotiation_ctx_t *context,int width,int height,int birate)
{
	sdp_message_t *remote_sdp;
	sdp_message_t *local_sdp = NULL;
	int i;
	...
	if (MSG_IS_INVITE(request)||MSG_IS_OPTIONS(request)||MSG_IS_RESPONSE_FOR(request, "INVITE"))
	{
		...
		i = osip_negotiation_ctx_execute_negotiation2(eXosip.osip_negotiation, context,width,height,birate);
		if (i==200)
		{
			local_sdp = osip_negotiation_ctx_get_local_sdp(context);
...
/*重载此函数                                                                      */
int
eXosip_answer_invite_2xx2(eXosip_call_t *jc, eXosip_dialog_t *jd, int code, char *local_sdp_port, char *ctct, char *local_video_port,
						  char *public_sdp_port,  char *public_video_port,int width,int height,int bitrate)
{
	osip_event_t *evt_answer;
	osip_message_t *response;
	int i;
	...
	if(NULL != osip_list_get(&tr->orig_request->bodies,0))
	{
		body = generating_sdp_answer2(tr->orig_request, jc->c_ctx,width,height,bitrate);
		if (body==NULL)
			code = 488; /* bad sdp */
	}
	else
