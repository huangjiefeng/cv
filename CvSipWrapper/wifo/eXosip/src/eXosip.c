...
int eXosip_init(FILE *input, FILE *output, int udp_port, int tcp_port, int tls_port,int ipv6_enable)
{
	osip_t *osip;
	...
	//是否IPv6
	eXosip_enable_ipv6(ipv6_enable);
	if (ipv6_enable)
	{
 ...
 )
{
	...
	if (local_audio_sdp_port!=NULL)
	{
		...
		osip_negotiation_sdp_build_offer2(eXosip.osip_negotiation, NULL, &sdp, public_sdp_port, public_video_sdp_port,width,height,bitrate);
...
/*重载此函数*/
int eXosip_answer_call2(int jid, int status, char *local_sdp_port, char *local_video_port, char *public_sdp_port, char *public_video_port,int width,int height,int bitrate)
{
	int i = -1;
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	...
	else if (status>199 && status<300)
	{
		...
		i = eXosip_answer_invite_2xx2(jc, jd, status, local_sdp_port, contact, local_video_port, public_sdp_port, public_video_port,width,height,bitrate);
	}
	else if (status>300 && status<699)
	{
		i = eXosip_answer_invite_3456xx(jc, jd, status, contact);
...
//重载函数eXosip_retrieve_negotiated_video_payload，新增参数char *vfmtp
//因为C语言不支持函数重载，所以每个名字都会加上数字“2”，表示重载原来的函数
int
eXosip_retrieve_negotiated_video_payload2(int jid, int *payload,char *vfmtp ,char *payload_name, int pnsize)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	int pl;
	...
	pl = eXosip_retrieve_sdp_negotiation_video_result2(jc->c_ctx, vfmtp,payload_name, pnsize);
	/************************************************************************/
	//在jresponse.c那里本要释放remote_sdp，因为后面还会用到，所以用完后在这里释放
	remote_sdp = osip_negotiation_ctx_get_remote_sdp(jc->c_ctx);
	sdp_message_free(remote_sdp);
	osip_negotiation_ctx_set_remote_sdp(jc->c_ctx, NULL);
	/************************************************************************/
...
int eXosip_register      (int rid, int registration_period,const char* content, const char* mime)
{
	osip_transaction_t *transaction;
	osip_event_t *sipevent;
	osip_message_t *reg;
	...
	if (reg==NULL)
	{
		if( content!=0&&mime!=0 )
		{
			i = generating_register_with_message(&reg, jr->r_aor, jr->r_registrar, 
				jr->r_contact, jr->r_reg_period, jr->r_route, ++jr->r_seq, content , mime );
		}else
		{
 ...
 * @author zhenHua.sun
 * @date 2011-06-15
 * @brief 往SIP注册消息中写入content
 * @param rid .
 * @param expires The expires value for registration.
 * @param content 消息体中的内容
 * @param mime content对应的mime类型
 */
int	  eXosip_register_with_content (int rid , int expires , const char* content, const char* mime)
{
...
//coolview
	if (winfo==-1) {
		owsip_message_set_header(subscribe,"Event","coolview");
		owsip_message_set_header(subscribe,"Accept","application/coolview");
	}
//coolview
...
/************************************************************************/
/*just for coolview*///add by zhuang 09-03-31
int eXosip_subscribe_close_4coolview(int sid)
{
	int i;
	eXosip_dialog_t *jd = NULL;
	eXosip_subscribe_t *js = NULL;
	...
	i = eXosip_subscribe_send_subscribe_4coolview(js, jd, "0");
	return i;
}
/************************************************************************/
