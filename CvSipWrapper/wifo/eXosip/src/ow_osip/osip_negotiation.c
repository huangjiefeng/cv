      ...
      else if (0 == strncmp (tmp, "video", 5))
	{
	  ...
	  do
	      ...
	      if (payload != NULL)
		{
		  ...
		  /************************************************************************/
		  /* 
		  __payload_t *my = (__payload_t *) osip_list_get (config->video_codec, 0);
		  if(strcmp(my->payload,my_payload->payload)==0)
		  开始执行下面代码
		  */
		  /************************************************************************/
		  if (my_payload != NULL)	/* payload is supported */
		      ...
		      if (0 == ret)
			{
			  ...
			  //构造协商之后的本机SDP，这里的fmtp应设置本机摄像头的分辨率，BitRate
			  if (my_payload->a_rtpmap != NULL)
			  {
				  med = (sdp_media_t*) osip_list_get(&remote->m_medias, i);
				  for (pos_attr=0;!osip_list_eol(&med->a_attributes, pos_attr);pos_attr++)
				  {
					  ...
					  if (0==osip_strncasecmp(attr->a_att_field, "fmtp", 4))
					  {
						  ...
						  {
							  sdp_message_a_attribute_add (*dest, i,osip_strdup("fmtp"),osip_strdup(attr->a_att_value)); 
							  break;
						  }
					  }
				  }
...
static int
sdp_confirm_media2 (osip_negotiation_t * config,
					osip_negotiation_ctx_t * context, sdp_message_t * remote,
					sdp_message_t ** dest,int width,int height,int bitrate)
{
	char *payload;
	char *tmp, *tmp2, *tmp3, *tmp4;
	char* local_video_port;
					...
					if (payload != NULL)
					{
						...
						if (my_payload != NULL)	/* payload is supported */
						{
							...
							if (0 == ret)
							{
								...
								if (my_payload->a_rtpmap != NULL)
								{
									char *fmtpValue = malloc(50);
									sprintf (fmtpValue, "%s CUSTOM=%d,%d MaxBR=%d", payload,width,height,bitrate);
									sdp_message_a_attribute_add (*dest, i,osip_strdup("fmtp"),osip_strdup(fmtpValue)); 
...
/*重载此函数，传入width，height，bitrate                                */
int
osip_negotiation_ctx_execute_negotiation2 (osip_negotiation_t * config,
										   osip_negotiation_ctx_t * context,
										   int width,int height,int bitrate)
{
	...
	i = sdp_confirm_media2 (config, context, remote, &local,width,height,bitrate);
	if (i != 0)
	{
		sdp_message_free (local);
		return i;
...
/************************************************************************///Add by zhuang 08-12-23
  int frame_width = 176,frame_height=144,bitrate=1960;
/************************************************************************/
  int i;
  int media_line = 0;
  const char *ptime = getenv("EXOSIP_FORCE_PTIME");
  ...
  if (!osip_list_eol (config->video_codec, 0))
	...
	(__payload_t *) osip_list_get (config->video_codec, pos);
	  ...
	  while (!osip_list_eol (config->video_codec, pos))
	  {
		  ...
		  {
			  char *fmtpValue = malloc(50);
			  sprintf (fmtpValue, "%s CUSTOM=%d,%d MaxBR=%d FPS=%d", my->payload,frame_width,frame_height,bitrate , config->video_fps);
			  sdp_message_a_attribute_add (*sdp, media_line,
				  osip_strdup ("fmtp"),
				  osip_strdup (fmtpValue));  
			  free(fmtpValue);
		  }
...
/* 重载此函数，传入width，height，bitrate  */
int
osip_negotiation_sdp_build_offer2 (osip_negotiation_t * config,
								  osip_negotiation_ctx_t * con,
								  sdp_message_t ** sdp, char *audio_port,
								  char *video_port,int frame_width,int frame_height,int bitrate)
{
	int i;
	int media_line = 0;
	const char *ptime = getenv("EXOSIP_FORCE_PTIME");
	...
	if (!osip_list_eol (config->video_codec, 0))
	{
		...
		while (!osip_list_eol (config->video_codec, pos))
		{
			...
			{
				char *fmtpValue = malloc(50);
				sprintf (fmtpValue, "%s CUSTOM=%d,%d MaxBR=%d", my->payload,frame_width,frame_height,bitrate);
				sdp_message_a_attribute_add (*sdp, media_line,
					osip_strdup ("fmtp"),
					osip_strdup (fmtpValue));  
				free(fmtpValue);
...
/************************************************************************/
/*设置SDP中的宽和高*/
int osip_negotiation_set_video_width_height (osip_negotiation_t * config, const int width,const int height, const int fps )
{
	if (config == NULL)
		return -1;
