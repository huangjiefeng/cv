...
int eXosip_get_sdp_media_info2(sdp_message_t *sdp, sdp_message_t *remote_sdp,const char *mtype, char *fmtp,char *payload_name,  int pnsize, char *ipaddr, int ipsize, int *port, int *ptime)
{
	sdp_media_t *med; 
	...
	if(remote_sdp)
	{
		for(n = 0; ; n++) 
		{
			remote_med = (sdp_media_t*) osip_list_get(&remote_sdp->m_medias, n);

			if (!strcmp(remote_med->m_media, mtype))
				break;
		}
	}
	else
	...
	if (payload!=NULL && payload_name!=NULL)
	{
		...
		//get fmtp
		for (pos_attr=0;!osip_list_eol(&remote_med->a_attributes, pos_attr);pos_attr++)
		{
			sdp_attribute_t *attr;
			attr = (sdp_attribute_t *)osip_list_get(&remote_med->a_attributes, pos_attr);

			if (0==osip_strncasecmp(attr->a_att_field, "fmtp", 4))
			{
				if ((payload_result <10 && 
					0==osip_strncasecmp(attr->a_att_value, payload, 1))
					||(payload_result>9 && payload_result<100 && 
					...
					0==osip_strncasecmp(attr->a_att_value, payload, 3)))
				{
					snprintf(fmtp, 128, "%s", attr->a_att_value);
					break;
				}
			}			
		}
...
int eXosip_get_sdp_media_format_info(sdp_message_t *sdp, eXosip_event_t *je)
{
	...
	for (pos = 0; !sdp_message_endof_media(sdp, pos); pos++)
	{
		...
		/************************************************************************///Add by zhuang08-12-19 
		// Looking for the video media field
		else if (osip_strncasecmp(tmp, "video", 5) == 0)
		{
			int i;
			sdp_attribute_t *att;
			int pnum = je->payload;
			char *vpayload = malloc(50);
			snprintf(vpayload, 50, "%d",je->payload);
			for (i = 0; (att = sdp_message_attribute_get(sdp, pos, i)); i++)
			{
				// Looking for the ptime attribute in the audio media field
				if (strcmp("fmtp", att->a_att_field) == 0)
				{
					if ((pnum <10 && 
						0==osip_strncasecmp(att->a_att_value, vpayload, 1))
						||(pnum>9 && pnum<100 && 
						0==osip_strncasecmp(att->a_att_value, vpayload, 2))
						||(pnum >100 && pnum<128 &&
						0==osip_strncasecmp(att->a_att_value, vpayload, 3)))
					{
						if (!je->format_modifiers)
						{
							je->format_modifiers = (osip_list_t*) osip_malloc(sizeof(osip_list_t));
...
//重载函数，新增参数char *vfmtp
//因为C语言不支持函数重载，所以每个名字都会加上数字“2”，表示重载原来的函数
int eXosip_retrieve_sdp_negotiation_video_result2(osip_negotiation_ctx_t *ctx, char *vfmtp,char *payload_name,  int pnsize)
{
	return eXosip_retrieve_sdp_negotiation_result_for2(ctx, "video", vfmtp,payload_name, pnsize);
}

int eXosip_retrieve_sdp_negotiation_result_for2(osip_negotiation_ctx_t *ctx, const char *mtype, char *vfmtp,char *pname, int pnsize)
{
	...
	return eXosip_get_sdp_media_info2(osip_negotiation_ctx_get_local_sdp(ctx), osip_negotiation_ctx_get_remote_sdp(ctx),mtype, vfmtp,pname, pnsize, 0,0,0,0);
...
/************************************************************************/
/*设置SDP中的宽和高*/
int eXosip_sdp_negotiation_set_video_width_height (const int width,const int height,const int fps)
{
	return osip_negotiation_set_video_width_height(eXosip.osip_negotiation,width,height , fps);
}
/************************************************************************/