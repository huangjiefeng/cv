				    ...
				    osip_event_t *evt)
{
      ...
      if (ctype!=NULL && ctype->type!=NULL && ctype->subtype!=NULL)
	{
	  if (0==osip_strcasecmp(ctype->type, "application")
	      && ((0==osip_strcasecmp(ctype->subtype, "pidf+xml")) || (0==osip_strcasecmp(ctype->subtype, "watcherinfo+xml"))
		      ||(0==osip_strcasecmp(ctype->subtype, "cpim-pidf+xml"))||(0==osip_strcasecmp(ctype->subtype, "coolview"))))
	    osip_message_get_body(evt->sip, 0, &body);
	  else
	    {
...

  {
    ...
    osip_message_header_get_byname(evt->sip, "event", 0, &event_hdr);
    if ( event_hdr==NULL || event_hdr->hvalue==NULL || 
	 (strcmp(event_hdr->hvalue,"coolview")&&strcmp(event_hdr->hvalue,"presence") && strcmp(event_hdr->hvalue,"presence.winfo")))
      {
...
eXosip_recv (OWSLSocket socket)
{
	...
	//msgBuf = osip_malloc (SIP_MESSAGE_MAX_LENGTH * sizeof (char) + 1) ;
	...
	if( i > 5 ) /* we expect at least one byte, otherwise there's no doubt that it is not a sip message !*/
	{
		...
		/*printf ("RECEIVED:\n%s\n", buffer) ;*/
		/************************************************************************/
		/*考虑到可能一个TCP包中含有多个sip消息，采用循环解析*/
		//temp = buffer;
		//temp2 = strstr(temp,"\n\r\nSIP/2.0");
		//
		//while (1)
		//{
		//	memset(msgBuf,0,SIP_MESSAGE_MAX_LENGTH * sizeof (char) + 1);
		//	if (temp2==NULL)
		//	{
		//		strcpy(msgBuf,temp);
		//	}
		...
		//	{
		//		temp2 = temp2+sizeof("\n\r\n")-1;
		//		strncpy(msgBuf,temp,temp2-temp);
		//	}
		//	OSIP_TRACE(osip_trace(__FILE__,__LINE__,OSIP_ERROR,NULL,
		//		"buf is:%s\n", msgBuf));
		//	sipevent = osip_parse (msgBuf, strlen(msgBuf)+1) ;
		...
		if (sipevent != NULL && sipevent->sip != NULL)
		{
			...
			/* TODO: fix it for IPv6 */
			//osip_message_fix_last_via_header (sipevent->sip, inet_ntoa (((struct sockaddr_in *) & address)->sin_addr), ntohs (((struct sockaddr_in *) & address)->sin_port)) ;
			getnameinfo ((struct sockaddr_in *) & address,address_length, ip, sizeof(ip), NULL, 0, NI_NUMERICHOST);
			osip_message_fix_last_via_header (sipevent->sip, ip, ntohs (((struct sockaddr_in *) & address)->sin_port)) ;
