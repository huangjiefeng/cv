 ...
 * Answer call.
 * 
 * @param jid          dialog id of the subscribe
 * @param status       Status code to use.
 */
int   eXosip_answer_subscribe   (int jid, int status);
...
/**
 * Retreive the answer we've got to this registration message
 * 
 * @param rid       Registration id
 * @param ans       returned answer message
 * @return          0  if there is valid answer
 */
 ...
 * @brief 在2011-06-15加入两个新的参数,content 和mime
 * @param rid .
 * @param expires The expires value for registration.
 * @param content 消息体中的内容
 * @param mime content对应的mime类型
 */
int   eXosip_register      (int rid, int expires,const char* content, const char* mime);
