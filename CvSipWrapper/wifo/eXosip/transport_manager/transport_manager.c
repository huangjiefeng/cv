...
)
{
	...
	getaddrinfo(NULL, "12345", &hints, &res); /* new api to get address information */
	/************************************************************************/
	//if (owsl_address_public_ip_get (address_family, public_ip, sizeof (public_ip)) == 0)
	if (owsl_address_public_ip_get2 (address_family, public_ip, sizeof (public_ip)) == 0)
	{
		ip = public_ip ;
	}
	...
	if (address != NULL)
	{
		bound_address = transport_address_listen (protocol, address, /*OWSL_ADDRESS_SIZE sizeof(struct sockaddr)*/res->ai_addrlen, 0, pending_max) ;
		free (address) ;
	}
